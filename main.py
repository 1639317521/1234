...
COMFYUI_HISTORY_TIMEOUT = int(float(os.getenv("COMFYUI_HISTORY_TIMEOUT", "1200")))
...
def provider_endpoint_url(provider, key, default_path):
    ...
    # 兼容：base_url 以 /vN 结尾，default_path 以不同版本号开头（如 /v1beta），
    # 去掉 base_url 的版本号后缀再拼接，避免双版本号（如 /v1/v1beta）
...
def normalize_provider_key_notes(rows):
    out = []
    for it in (rows or []):
        key = str((it or {}).get("key") or "").strip()
        if not key:
            continue
        note = re.sub(r"\s+", " ", str((it or {}).get("note") or "").strip())[:60]
...
def normalize_provider(item):
    ...
    **(
        {
            "wallet_keys": normalize_provider_key_notes(item.get("wallet_keys")),
            "free_keys": normalize_provider_key_notes(item.get("free_keys")),
        }
        if provider_id == "runninghub" else {}
    ),
...
def public_provider(provider):
    ...
    if provider.get("id") == "runninghub":
        ...
        wallet_candidates = runninghub_wallet_key_candidates(provider)
        free_candidates = runninghub_key_candidates(provider, prefer_wallet=False)
        item.update({
            "has_wallet_key": bool(wallet_key),
            "wallet_key_preview": mask_secret(wallet_key),
            "wallet_key_env": runninghub_wallet_key_env(),
            "wallet_keys_count": len(wallet_candidates),
            "wallet_keys": runninghub_annotate_candidates_with_rotation(wallet_candidates, "wallet"),
            "free_keys_count": len(free_candidates),
            "free_keys": runninghub_annotate_candidates_with_rotation(free_candidates, "free"),
        })
...
def check_update():
    ...
    item["source"] = "github"
    github = item or {"version": "", "ok": False, "error": "检测超时（超过 5s）", "url": GITHUB_VERSION_URL, "source": "github"}
    best: Dict[str, Any] = {}
...
def stage_update_from_source(source: str, staging_root: str) -> Tuple[List[str], List[str], List[str]]:
    """下载 GitHub 源更新文件到 staging，返回 (root_files, static_files, files)。失败抛异常。"""
...
def download_comfy_output(comfy_address, item, prefix="studio_"):
    ...
    full_url = f"http://{comfy_address}{comfy_url_path}"
    # 瞬时网络抖动/后端刚忙完时重试（最多 3 次），避免留下 0 字节文件并回退远程 URL 导致画布视频打不开
    for attempt in range(3):
        try:
            ...
            if os.path.getsize(local_path) > 0:
                return output_url_for(filename, "output")
            print(f"下载 ComfyUI 输出为空(attempt {attempt + 1}): {full_url}")
        except Exception as e:
            print(f"下载 ComfyUI 输出失败(attempt {attempt + 1}): {e}")
        time.sleep(2)
    # 全部失败才回退远程 URL（画布视频走 /api/view 后端代理播放）
    try:
...
def get_comfy_history(comfy_address, prompt_id):
    try:
        with urllib.request.urlopen(f"http://{comfy_address}/history/{prompt_id}", timeout=3) as response:
            return json.loads(response.read())
    except Exception as e:
        return {}

def comfy_queue_status(comfy_address, prompt_id):
    """查询云端 ComfyUI 队列里该 prompt 的状态：'running' 执行中 / 'pending' 排队中 / None 不在队列。
    网络异常会抛异常（由调用方区分，不误判为任务丢失）。
    ComfyUI 0.3x 队列项结构：[number, prompt_id, prompt, extra_data, outputs_to_execute]，
    老版本可能 [prompt_id, number, ...]，所以 [0]/[1] 都查。"""
    with urllib.request.urlopen(f"http://{comfy_address}/queue", timeout=3) as response:
        data = json.loads(response.read())
    for entry in data.get("queue_running", []) or []:
        if isinstance(entry, list) and len(entry) >= 2 and str(prompt_id) in (str(entry[0]), str(entry[1])):
            return "running"
...
def comfy_execution_error_message(entry, workflow_nodes=None):
    """从 ComfyUI history 条目解析 execution_error 事件，组合成可读错误。"""
    workflow_nodes = workflow_nodes or {}
    status = entry.get("status") or {}
    messages = status.get("messages") or []
    parts = []
    for msg in messages:
        if not msg or len(msg) < 2 or msg[0] != "execution_error":
            continue
        data = msg[1] or {}
        exc_type = str(data.get("exception_type") or "").strip()
        exc_msg = str(data.get("exception_message") or "").strip()
        node_id = str(data.get("node_id") or "").strip()
        detail = exc_msg or exc_type or "未知执行错误"
        if exc_type and exc_msg:
            detail = f"{exc_type}: {exc_msg}"
        if node_id:
            node_def = workflow_nodes.get(node_id) if isinstance(workflow_nodes, dict) else None
            label = ""
            if isinstance(node_def, dict):
                label = str(node_def.get("_meta", {}).get("title") or node_def.get("class_type") or "")
            parts.append(f"节点 {node_id}" + (f"（{label}）" if label else "") + f"：{detail[:300]}")
        else:
    ...
    if not parts:
        trace = ""
        for msg in messages:
            if msg and len(msg) > 1 and msg[0] == "execution_error":
                trace = str((msg[1] or {}).get("traceback") or "")[:400]
                break
        parts.append(trace or "云端 ComfyUI 节点执行出错")
    return "ComfyUI 工作流执行失败：" + "；".join(parts[:3])

# --- ComfyUI 云端 WebSocket 实时进度（方案 A：实时显示当前节点/采样进度） ---
# 说明：后端用与提交 prompt 相同的 client_id 连云端 /ws，收到 executing/progress/executed
# 事件后按 prompt_id 匹配画布任务并写入进度；前端仍走 1s 轮询取数（断线自动降级回轮询）。
_COMFY_WS_TASKS = {}          # addr -> asyncio.Task
_COMFY_NODE_TITLE_CACHE = {}  # workflow_json -> {node_id: title}
_COMFY_NODE_COUNT_CACHE = {}  # workflow_json -> 节点数

def comfy_workflow_node_info(workflow_name):
    """加载工作流 JSON，返回 (title_map, node_count)，带缓存。"""
    key = workflow_name or ""
    if key in _COMFY_NODE_TITLE_CACHE:
        return _COMFY_NODE_TITLE_CACHE[key], _COMFY_NODE_COUNT_CACHE.get(key, 0)
    nodes = {}
    try:
        path = os.path.join(WORKFLOW_DIR, key)
        if os.path.exists(path):
            with open(path, "r", encoding="utf-8") as f:
                wf = json.load(f)
            if isinstance(wf, dict):
                nodes = {str(nid): node for nid, node in wf.items() if isinstance(node, dict)}
    except Exception:
        pass
    titles = {}
    for nid, node in nodes.items():
        meta = node.get("_meta") if isinstance(node, dict) else None
        titles[nid] = str((meta or {}).get("title") or node.get("class_type") or "")
    _COMFY_NODE_TITLE_CACHE[key] = titles
    _COMFY_NODE_COUNT_CACHE[key] = len(nodes)
    return titles, len(nodes)

def comfy_node_title(workflow_name, node_id):
    titles, _ = comfy_workflow_node_info(workflow_name)
    return titles.get(str(node_id), "")

async def comfy_ws_listener(addr):
    uri = f"ws://{addr}/ws?clientId={CLIENT_ID}"
    backoff = 3
    while True:
        try:
            async with websockets.connect(uri, ping_interval=20, ping_timeout=20, open_timeout=10, max_size=16 * 1024 * 1024) as ws:
                backoff = 3
                async for raw in ws:
                    try:
                        msg = json.loads(raw)
                    ...
                    if not isinstance(msg, dict):
                        continue
                    mtype = msg.get("type")
                    if mtype not in ("executing", "progress", "executed", "execution_cached", "execution_error", "execution_success"):
                        continue
                    ...
                    with CANVAS_TASK_LOCK:
                        task = next((t for t in CANVAS_TASKS.values() if str(t.get("prompt_id") or "") == prompt_id), None)
                        if task is None:
                            continue
                        if mtype == "executing":
                            node = data.get("node")
                            if node:
                                task["stage"] = "running"
                                task["current_node"] = str(node)
                                task["current_node_title"] = comfy_node_title(task.get("workflow_json", ""), str(node))
                            else:
                                task["current_node"] = ""
                                task["current_node_title"] = ""
                            task["updated_at"] = time.time()
                        elif mtype == "progress":
                            value, maxv = data.get("value"), data.get("max")
                            if isinstance(value, (int, float)) and isinstance(maxv, (int, float)) and maxv:
                                task["sampler"] = {"value": int(value), "max": int(maxv)}
                                task["updated_at"] = time.time()
                        elif mtype == "executed":
                            task["executed_nodes"] = int(task.get("executed_nodes") or 0) + 1
                            task["updated_at"] = time.time()
            ...
            print(f"ComfyUI WS 监听 {addr} 断开（{e}），{backoff}s 后重连")
            await asyncio.sleep(backoff)
            backoff = min(backoff * 2, 30)

def start_comfy_ws_listeners():
    loop = GLOBAL_LOOP or asyncio.get_event_loop()
    for addr in COMFYUI_INSTANCES:
        key = str(addr)
        if key in _COMFY_WS_TASKS and not _COMFY_WS_TASKS[key].done():
            continue
        try:
            _COMFY_WS_TASKS[key] = loop.create_task(comfy_ws_listener(addr))
        except Exception as e:
            print(f"启动 ComfyUI WS 监听失败 {addr}: {e}")
...
def resolve_chat_provider(provider: str, model: str, ms_model: str):
    if provider == "modelscope":
        ...
        hdrs = {"Authorization": bearer_auth_value(clean_token), "Content-Type": "application/json", "User-Agent": BROWSER_UA}
...
# 浏览器 User-Agent：部分 API 站套 Cloudflare 反爬（error 1010），拦截 httpx/urllib 默认签名；
# 统一携带浏览器 UA 以兼容此类站点
...
async def httpx_request_with_transient_retries(client, method, url, attempts=2, retry_delay=1.2, retry_read_break=False, **kwargs):
    attempts = max(1, int(attempts or 1))
    last_exc = None
    retry_statuses = {502, 503, 504, 520, 522, 524}
    # 连接类异常（请求未送达上游）重发无计费风险，总是重试；
    # 读断类异常（RemoteProtocolError/ReadError/ReadTimeout：请求已送达、响应中断）重发可能导致上游重复计费，
    # 仅在调用方明确允许（retry_read_break=True）时才重试。
    always_retry_exc = (httpx.ConnectError, httpx.ConnectTimeout, httpx.PoolTimeout)
    read_break_exc = (httpx.RemoteProtocolError, httpx.ReadError, httpx.ReadTimeout)
    for attempt in range(attempts):
        try:
            ...
            if response.status_code in retry_statuses and attempt + 1 < attempts:
                ...
                continue
            return response
        except always_retry_exc as exc:
            last_exc = exc
            if attempt + 1 >= attempts:
                raise
            print(f"[HTTPX-RETRY] {method} {url} transient error: {exc}; retry {attempt + 2}/{attempts}", flush=True)
            await asyncio.sleep(retry_delay * (attempt + 1))
        except read_break_exc as exc:
            if not retry_read_break or attempt + 1 >= attempts:
                raise
            last_exc = exc
            print(f"[HTTPX-RETRY] {method} {url} read-break error: {exc}; retry {attempt + 2}/{attempts}", flush=True)
            await asyncio.sleep(retry_delay * (attempt + 1))
    if last_exc:
        raise last_exc
...
RH_TASK_WALLET_KEY: Dict[str, Dict[str, str]] = {}  # taskId -> {"key","note"}（提交成功所用钱包钥匙，供查询沿用）

def runninghub_wallet_key_candidates(provider=None):
    """钱包密钥候选列表 [{key, note}]：优先 wallet_keys 数组；数组为空时兼容旧单 key 字段与 env。按序去重。"""
    provider = provider or runninghub_provider()
    out, seen = [], set()
    for it in (provider.get("wallet_keys") or []):
        k = str((it or {}).get("key") or "").strip()
        if not k or k in seen:
            continue
        seen.add(k)
        out.append({"key": k, "note": str((it or {}).get("note") or "").strip()})
    if out:
        return out
    legacy = str((provider or {}).get("wallet_api_key") or "").strip() or runninghub_wallet_key_value()
    if legacy:
        out.append({"key": legacy, "note": ""})
    return out

def runninghub_free_key_candidates(provider=None):
    """RH币(免费额度)密钥候选 [{key, note}]：优先 free_keys 数组，空则回退旧 api_key/env。"""
    provider = provider or runninghub_provider()
    provider_id = (provider or {}).get("id") or "runninghub"
    out, seen = [], set()
    for it in (provider.get("free_keys") or []):
        k = str((it or {}).get("key") or "").strip()
        if not k or k in seen:
            continue
        seen.add(k)
    ...
    if out:
        return out
    legacy = str((provider or {}).get("api_key") or "").strip() or provider_env_key_value(provider_id)
    if legacy:
        out.append({"key": legacy, "note": ""})
    return out

def runninghub_key_candidates(provider=None, prefer_wallet=True):
    return runninghub_wallet_key_candidates(provider) if prefer_wallet else runninghub_free_key_candidates(provider)

# --- RH 多密钥游标轮换 + 每日成功计数 ---
# 状态文件：data/runninghub_key_rotation.json
# {"version":1,"updated_at":...,"free":{"cursor":N},"wallet":{"cursor":N},"daily":{"YYYY-MM-DD":{"<key指纹>":{"count":X,"note":"..."}}}}
# cursor 指向「下一把要用的钥匙」在候选列表中的下标；每次提交成功后 +1（取模循环），重启不丢。
RH_ROTATION_FILE = os.path.join(DATA_DIR, "runninghub_key_rotation.json")
RH_ROTATION_LOCK = Lock()

def rh_key_tail(key: str) -> str:
    """钥匙指纹：sha1 前 10 位，避免明文落盘且与顺序无关。"""
    return hashlib.sha1(str(key or "").encode("utf-8")).hexdigest()[:10]

def rh_rotation_today() -> str:
    return time.strftime("%Y-%m-%d")

def load_runninghub_rotation():
    try:
        with open(RH_ROTATION_FILE, "r", encoding="utf-8") as f:
            state = json.load(f)
        if not isinstance(state, dict):
            raise ValueError("bad rotation state")
    except Exception:
        state = {}
    state.setdefault("version", 2)
    for kind in ("free", "wallet"):
        entry = state.get(kind)
        if not isinstance(entry, dict):
            entry = {}
        try:
            entry["cursor"] = int(entry.get("cursor") or 0)
        except Exception:
            entry["cursor"] = 0
        keys = entry.get("keys")
    ...
    for date_key, day in daily.items():
        ...
        for kind in ("free", "wallet"):
            ...
            if isinstance(bucket, dict):
                clean_day[kind] = {t: v for t, v in bucket.items() if isinstance(v, dict)}
        if clean_day:
            clean_daily[str(date_key)] = clean_day
    state["daily"] = clean_daily
    return state

def save_runninghub_rotation(state):
    os.makedirs(os.path.dirname(RH_ROTATION_FILE), exist_ok=True)
    tmp_path = RH_ROTATION_FILE + ".tmp"
    with open(tmp_path, "w", encoding="utf-8") as f:
        json.dump(state, f, ensure_ascii=False, indent=2)
    os.replace(tmp_path, RH_ROTATION_FILE)

def runninghub_rotate_pick(provider=None, kind="free"):
    """游标轮换 + 自动跳过手动禁用的钥匙。
    返回 (candidates, cursor_start, try_indexes, all_disabled)：
    - candidates: 完整候选列表（含禁用的，供展示与下标定位）
    - try_indexes: 本次要依次尝试的**完整列表下标**序列——从游标位置起顺时针扫一整圈，跳过 disabled
    - all_disabled: 候选非空但全部被手动禁用
    """
    candidates = runninghub_key_candidates(provider, prefer_wallet=(kind == "wallet"))
    if not candidates:
        return [], 0, [], False
    with RH_ROTATION_LOCK:
        state = load_runninghub_rotation()
    key_states = (state.get(kind) or {}).get("keys") or {}
    def _enabled(idx):
        item = key_states.get(rh_key_tail(candidates[idx].get("key"))) or {}
        return item.get("enabled") is not False
    total = len(candidates)
    cursor = int((state.get(kind) or {}).get("cursor") or 0)
    start = cursor % total
    try_indexes = [i for i in [(start + step) % total for step in range(total)] if _enabled(i)]
    return candidates, start, try_indexes, not try_indexes

def rh_classify_key_failure(reason):
    text = str(reason or "").lower()
    if any(word in text for word in ("余额", "balance", "quota", "credit", "insufficient")):
        return "insufficient"
    return "error"

def rh_mark_key_status(kind, key, reason):
    """提交被拒（HTTP/业务拒绝）时标记钥匙状态；网络异常不调用此函数。"""
    tail = rh_key_tail(key)
    now = now_ms()
    with RH_ROTATION_LOCK:
        state = load_runninghub_rotation()
        keys = state.setdefault(kind, {}).setdefault("keys", {})
        item = keys.get(tail) if isinstance(keys.get(tail), dict) else {}
        item["enabled"] = item.get("enabled") is not False
        item["status"] = rh_classify_key_failure(reason)
        item["error"] = str(reason or "").strip()[:160]
        item["at"] = now
        keys[tail] = item
        state["updated_at"] = now
        save_runninghub_rotation(state)

def runninghub_set_key_enabled(kind, key, enabled):
    tail = rh_key_tail(key)
    now = now_ms()
    with RH_ROTATION_LOCK:
        state = load_runninghub_rotation()
        ...
        # 重新启用时若没有失败记录，补一个空状态占位，保证行数据稳定
        item.setdefault("status", "")
        item.setdefault("error", "")
        item.setdefault("at", 0)
        keys[tail] = item
        state["updated_at"] = now
        save_runninghub_rotation(state)

def runninghub_clear_key_status(kind):
    """一键清除：清该类全部钥匙的 status/error/at，并把当日成功计数归零。游标与禁用开关保留。"""
    today = rh_rotation_today()
    with RH_ROTATION_LOCK:
        state = load_runninghub_rotation()
...
def runninghub_rotate_commit(kind, abs_index, total, key, note=""):
    """一次提交成功后调用：游标推进到下一把（取模循环），当日该钥匙成功次数 +1，并清除其失败状态。"""
    tail = rh_key_tail(key)
    today = rh_rotation_today()
    now = now_ms()
    with RH_ROTATION_LOCK:
        ...
        entry = state.setdefault(kind, {})
        try:
            entry["cursor"] = (int(abs_index) + 1) % max(int(total) or 1, 1)
        except Exception:
            entry["cursor"] = 0
        ...
        daily = state.setdefault("daily", {})
        day = daily.get(today) if isinstance(daily.get(today), dict) else {}
        bucket = day.get(kind) if isinstance(day.get(kind), dict) else {}
        counter = bucket.get(tail) if isinstance(bucket.get(tail), dict) else {}
        counter["count"] = int(counter.get("count") or 0) + 1
        counter["note"] = (note or counter.get("note") or "").strip()
        bucket[tail] = counter
        day[kind] = bucket
        daily[today] = day
        state["daily"] = dict(sorted(daily.items())[-3:])
        state["updated_at"] = now
        save_runninghub_rotation(state)

def runninghub_annotate_candidates_with_rotation(candidates, kind):
    """给候选列表附加 today_success / is_next / enabled / status / error 字段，供设置界面展示。"""
    total = len(candidates)
    if not total:
        return candidates
    state = load_runninghub_rotation()
    cursor = int((state.get(kind) or {}).get("cursor") or 0)
    key_states = (state.get(kind) or {}).get("keys") or {}
    today_map = ((state.get("daily") or {}).get(rh_rotation_today()) or {}).get(kind) or {}

    def _is_enabled(index):
        info = key_states.get(rh_key_tail(candidates[index].get("key"))) or {}
        return info.get("enabled") is not False

    next_index = -1
    for step in range(total):
        i = (cursor % total + step) % total
        if _is_enabled(i):
            next_index = i
            break
    ...
    for index, cand in enumerate(candidates):
        tail = rh_key_tail(cand.get("key"))
        kinfo = key_states.get(tail) or {}
        cinfo = today_map.get(tail) or {}
        out.append({
            **cand,
            "today_success": int(cinfo.get("count") or 0),
            "is_next": index == next_index,
            "enabled": kinfo.get("enabled") is not False,
            "status": str(kinfo.get("status") or ""),
            "error": str(kinfo.get("error") or ""),
            "status_at": int(kinfo.get("at") or 0),
        })
    return out

def runninghub_task_api_key_v2(task_id, kind="wallet"):
    info = RH_TASK_WALLET_KEY.get(str(task_id or "")) or {}
    if info.get("key"):
        return info["key"]
    return None

def runninghub_wallet_note_for(key, provider=None):
    key = str(key or "")
    for it in runninghub_wallet_key_candidates(provider):
        if it["key"] == key:
            return it["note"]
    return ""

def runninghub_task_api_key(task_id, provider=None, use_wallet=False):
    """状态/结果查询取钥匙：优先提交成功时记录的那把（多密钥轮换场景），否则回退默认逻辑。"""
    info = RH_TASK_WALLET_KEY.get(str(task_id or "")) or {}
    if info.get("key"):
        return info["key"]
    return runninghub_api_key(provider, use_wallet=use_wallet)

def rh_bearer_headers(api_key, json_body=True):
    headers = {"Authorization": bearer_auth_value(api_key), "Accept": "application/json", "User-Agent": BROWSER_UA}
    if json_body:
        headers["Content-Type"] = "application/json"
    return headers
...
async def generate_runninghub_entry_image(prompt, size, model, reference_images, provider, entry):
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        rot_candidates, _rot_cursor, rot_indexes, rot_all_disabled = runninghub_rotate_pick(provider, "free")
        if rot_all_disabled:
            raise HTTPException(status_code=400, detail="所有 RH币密钥已被手动禁用，请在 API 设置中启用后重试。")
        rot_index = rot_indexes[0] if rot_indexes else 0
        api_key = rot_candidates[rot_index]["key"] if rot_candidates else runninghub_api_key(provider, use_wallet=use_wallet)
        rot_note = str((rot_candidates[rot_index].get("note") if rot_candidates else "") or "")
        if kind == "workflow":
        ...
        if rot_candidates:
            runninghub_rotate_commit("free", rot_index, len(rot_candidates), api_key, rot_note)
    ...
    async with httpx.AsyncClient(timeout=request_timeout) as client:
        ...
        if image_request_mode == "openai-video-proxy":
            ...
            if quality:
                ...
                body["quality"] = "auto"
            # 仅对"连接未建立/未送达"类网络错误自动重试；响应中途断连不重发（避免上游重复计费），由上层给出取图指引
            response = await httpx_request_with_transient_retries(
                client, "POST", gen_url, attempts=3,
                headers=api_headers(provider=provider, model=model), json=body,
            )
...
@app.get("/api/view")
def view_image(request: Request, filename: str, type: str = "input", subfolder: str = ""):
    # 先按原逻辑去各 ComfyUI 后端找（流式转发 + Range 断点支持，图片/视频都能播）
    for addr in COMFYUI_INSTANCES:
        try:
            ...
            r = requests.get(url, params=params, headers=upstream_headers, timeout=30, stream=True)
            if r.status_code == 200 or r.status_code == 206:
                def _stream():
                    try:
                        ...
                        r.close()
                resp = StreamingResponse(_stream(), media_type=r.headers.get('Content-Type'))
                resp.status_code = r.status_code
                for key in ("content-range", "accept-ranges", "content-length", "content-disposition"):
...
def open_output_folder(payload: OpenFolderRequest):
    """打开生成结果所在的本地文件夹（资源管理器定位到该文件）。仅支持本地缓存文件。"""
    url = (payload.url or "").strip()
    path = output_file_from_url(url)
    if not path:
        name = filename_from_media_url(url, "")
        if name:
            path = local_media_file_by_basename(name)
    if not path or not os.path.isfile(path):
        raise HTTPException(status_code=404, detail="文件不在本地缓存，无法打开所在文件夹")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"打开所在文件夹失败：{e}")
...
async def runninghub_submit(payload: RunningHubSubmitRequest):
    ...
    if not webapp_id:
        raise HTTPException(status_code=400, detail="webappId 必填")
    provider = runninghub_provider()
    key_kind = "wallet" if payload.useWallet else "free"
    key_kind_cn = "余额密钥" if payload.useWallet else "RH币密钥"
    candidates, rotate_start, try_indexes, all_disabled = runninghub_rotate_pick(provider, key_kind)
    if all_disabled:
        raise HTTPException(status_code=400, detail=f"所有{key_kind_cn}已被手动禁用，请在 API 设置中启用，或一键清除状态后重试。")
    if not candidates:
        candidates = [{"key": runninghub_api_key(provider, use_wallet=payload.useWallet), "note": ""}]
        try_indexes = [0]
    url = runninghub_endpoint_url(provider, "/task/openapi/ai-app/run")
    attempts_log = []
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=180.0, write=120.0, pool=20.0)) as client:
        for cand_index in try_indexes:
            cand = candidates[cand_index]
            k, note = cand["key"], cand.get("note") or ""
            label = f"{key_kind_cn}[{note}]" if note else f"{key_kind_cn}[{mask_secret(k)}]"
            body = {
                "apiKey": k,
                "webappId": webapp_id,
            ...
            try:
                response = await client.post(url, headers=rh_bearer_headers(k), json=body)
                raw = response.json()
            except Exception as exc:
                attempts_log.append(f"{label} 网络错误：{exc}")
                log_runninghub_error("submit-network", {"error": str(exc)}, endpoint=url, webappId=webapp_id)
                continue
            if response.status_code >= 400:
                reason = (runninghub_fail_reason(raw) or response.text[:120]).strip()
                rh_mark_key_status(key_kind, k, f"HTTP {response.status_code}：{reason}")
                log_runninghub_error("submit-http", raw, endpoint=url, webappId=webapp_id, status=response.status_code)
                attempts_log.append(f"{label} HTTP {response.status_code}：{reason}")
                continue
            ...
            if code in (0, "0"):
                ...
                RH_TASK_WALLET_KEY[str(task_id)] = {"key": k, "note": note}
                runninghub_rotate_commit(key_kind, cand_index, len(candidates), k, note)
                if len(try_indexes) > 1 and attempts_log:
                    print(f"[RH-KEY-ROTATE] webappId={webapp_id} 前序尝试：{'；'.join(attempts_log)} → 第 {len(attempts_log)+1} 把成功", flush=True)
                return {"success": True, "data": {"taskId": task_id, "raw": raw}, "wallet_note": note}
            reason = (runninghub_fail_reason(raw) or "RunningHub 提交失败").strip()
            rh_mark_key_status(key_kind, k, reason)
            log_runninghub_error("submit-rejected", raw, endpoint=url, webappId=webapp_id)
            attempts_log.append(f"{label} 失败：{reason}")
    prefix = f"已依次尝试 {len(try_indexes)} 把{key_kind_cn}均失败：" if len(try_indexes) > 1 else ""
    raise HTTPException(status_code=400, detail=runninghub_error_detail(prefix + "；".join(attempts_log[-6:]), endpoint=url, webappId=webapp_id))
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=180.0, write=120.0, pool=20.0)) as client:
        for cand_index in try_indexes:
            ...
            try:
                ...
                attempts_log.append(f"{label} 网络错误：{exc}")
                log_runninghub_error("workflow-submit-network", {"error": str(exc)}, endpoint=url, workflowId=workflow_id)
                continue
            if response.status_code >= 400:
                reason = (runninghub_fail_reason(raw) or response.text[:120]).strip()
                rh_mark_key_status(key_kind, k, f"HTTP {response.status_code}：{reason}")
            ...
            if code in (0, "0"):
                ...
                runninghub_rotate_commit(key_kind, cand_index, len(candidates), k, note)
                if len(try_indexes) > 1 and attempts_log:
                    print(f"[RH-KEY-ROTATE] workflowId={workflow_id} 前序尝试：{'；'.join(attempts_log)} → 第 {len(attempts_log)+1} 把成功", flush=True)
                return {"success": True, "data": {"taskId": task_id, "raw": raw}, "wallet_note": note}
            reason = (runninghub_fail_reason(raw) or "RunningHub 工作流提交失败").strip()
            rh_mark_key_status(key_kind, k, reason)
            log_runninghub_error("workflow-submit-rejected", raw, endpoint=url, workflowId=workflow_id)
            attempts_log.append(f"{label} 失败：{reason}")
    prefix = f"已依次尝试 {len(try_indexes)} 把{key_kind_cn}均失败：" if len(try_indexes) > 1 else ""
    raise HTTPException(status_code=400, detail=runninghub_error_detail(prefix + "；".join(attempts_log[-6:]), endpoint=url, workflowId=workflow_id))
...
class RhKeyStateRequest(BaseModel):
    kind: str = "free"
    key: str = ""
    enabled: bool = True

class RhKeyStateClearRequest(BaseModel):
    kind: str = "free"

@app.post("/api/runninghub/key-state")
async def runninghub_set_key_state(payload: RhKeyStateRequest):
    """手动启用/禁用某一把 RH 密钥（按 RH币/余额列表独立）。"""
    kind = str(payload.kind or "free").strip().lower()
    if kind not in ("free", "wallet"):
        raise HTTPException(status_code=400, detail="kind 必须是 free 或 wallet")
    key = str(payload.key or "").strip()
    if not key:
        raise HTTPException(status_code=400, detail="key 必填")
    runninghub_set_key_enabled(kind, key, bool(payload.enabled))
    provider = runninghub_provider()
    candidates = runninghub_key_candidates(provider, prefer_wallet=(kind == "wallet"))
    return {"success": True, "keys": runninghub_annotate_candidates_with_rotation(candidates, kind)}

@app.post("/api/runninghub/key-state/clear")
async def runninghub_clear_key_state(payload: RhKeyStateClearRequest):
    """一键清除：该类全部钥匙的状态（余额不足/报错）清空，当日成功计数归零；游标与禁用开关保留。"""
    kind = str(payload.kind or "free").strip().lower()
    if kind not in ("free", "wallet"):
        raise HTTPException(status_code=400, detail="kind 必须是 free 或 wallet")
    runninghub_clear_key_status(kind)
    provider = runninghub_provider()
    candidates = runninghub_key_candidates(provider, prefer_wallet=(kind == "wallet"))
    return {"success": True, "keys": runninghub_annotate_candidates_with_rotation(candidates, kind)}
...
async def runninghub_query(taskId: str = "", useWallet: bool = False):
    ...
    provider = runninghub_provider()
    api_key = runninghub_task_api_key(task_id, provider, use_wallet=useWallet)
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=240.0, write=30.0, pool=20.0)) as client:
        try:
            response = await client.post(url, headers=rh_bearer_headers(api_key), json={"apiKey": api_key, "taskId": task_id})
            raw = response.json()
        except Exception as exc:
        ...
        image_items = []
        if status == "SUCCESS":
            # 兼容两种 outputs DTO：旧版 {code, data:{outputs:...}}，新版顶层直接 {status, results:...}
            data = raw.get("data") if isinstance(raw, dict) else None
...
def load_skill_package(pkg_dir, rel_id):
    """读取一个技能包：目录内 SKILL.md 的 frontmatter(name/description) + 正文作为系统提示词。"""
    md_path = os.path.join(pkg_dir, "SKILL.md")
    ...
    if text.startswith("---"):
        ...
        if len(parts) == 3:
            ...
            m_name = re.search(r"^\s*name\s*:\s*(.+)$", front, re.M)
            m_desc = re.search(r"^\s*description\s*:\s*(.+)$", front, re.M)
            if m_name:
                name = m_name.group(1).strip().strip("\"'")
            if m_desc:
                description = m_desc.group(1).strip().strip("\"'")
            prompt = body
    return {"id": rel_id, "name": name, "description": description, "prompt": prompt}


@app.get("/api/skills")
async def list_llm_skills():
    roots = [SKILLS_DIR] + [d.strip() for d in (os.environ.get("WUCANVAS_SKILL_DIRS") or "").split(";") if d.strip()]
    skills = []
    seen = set()
    for root in roots:
        ...
        for entry in sorted(os.listdir(root)):
            ...
            if os.path.isdir(pkg_dir) and os.path.isfile(os.path.join(pkg_dir, "SKILL.md")):
                seen.add(entry)
                try:
                    skills.append(load_skill_package(pkg_dir, entry))
                except Exception as e:
                    print(f"[skills] 加载失败 {pkg_dir}: {e}")
    ...
    for item in payload:
        ...
        if provider["id"] == "runninghub":
            ...
            if item.clear_wallet_key:
                ...
                provider["wallet_keys"] = []
                provider["wallet_api_key"] = ""
            elif provider.get("wallet_keys"):
                # 多密钥列表：第一把同步到旧字段/env，保持向后兼容
                first_key = str((provider["wallet_keys"][0] or {}).get("key") or "").strip()
                if first_key:
                    provider["wallet_api_key"] = first_key
                    env_updates[wallet_env] = first_key
            elif item.wallet_api_key is not None and item.wallet_api_key.strip():
                env_updates[wallet_env] = item.wallet_api_key.strip()
            free_env = provider_key_env("runninghub")
            if item.clear_key:
                env_updates[free_env] = ""
                provider["free_keys"] = []
                provider["api_key"] = ""
            elif provider.get("free_keys"):
                first_free = str((provider["free_keys"][0] or {}).get("key") or "").strip()
                if first_free:
                    provider["api_key"] = first_free
                    env_updates[free_env] = first_free
            elif item.api_key is not None and item.api_key.strip():
                env_updates[free_env] = item.api_key.strip()
...
async def build_online_image_result(payload: OnlineImageRequest):
    ...
    try:
        ...
        log_net_error(f"生图 响应中断 provider={provider.get('id')} model={model}", exc)
        raise HTTPException(
            status_code=502,
            detail=(
                "上游生成完成后、传回图片时连接被中断（incomplete chunked read）。"
                "这种情况下上游通常已经完成生成并计费：请先到 API 站后台（如 aicost 的订单/任务页）查看并下载本次生成的图片；"
                "确认没有出图后再点重试（直接重试会重新计费一次）。"
            ),
        ) from exc
    except httpx.HTTPError as exc:
...
async def run_canvas_comfy_task(task_id: str, payload: GenerateRequest):
    with CANVAS_TASK_LOCK:
        if task_id in CANVAS_TASKS:
            ...
            CANVAS_TASKS[task_id]["updated_at"] = time.time()
    def _progress(info):
        # 在 generate 的工作线程里被调用：把阶段/云端 prompt_id/backend 写进任务状态，供前端轮询展示与恢复
        with CANVAS_TASK_LOCK:
            task = CANVAS_TASKS.get(task_id)
            if not task:
                return
            for key in ("stage", "prompt_id", "backend"):
                if info.get(key):
                    task[key] = info[key]
            task["updated_at"] = time.time()
    try:
        result = await asyncio.to_thread(generate, payload, _progress)
...
async def get_canvas_comfy_task(task_id: str):
    ...
    # 附上工作流总节点数（前端算百分比用）；executed_nodes 由 WS 事件累计
    workflow_name = str(task.get("workflow_json") or "")
    if workflow_name:
        try:
            titles, total = comfy_workflow_node_info(workflow_name)
            if total:
                task["total_nodes"] = total
                current = task.get("current_node") or ""
                if current and not task.get("current_node_title"):
                    task["current_node_title"] = titles.get(current, "")
        except Exception:
            pass
    return task
...
async def get_canvas(canvas_id: str):
    canvas = load_canvas(canvas_id)
    # 删除墓碑：服务端记录过的已删节点，读取时直接过滤掉，弱网下"删了又被旧快照救活"不再发生
    tombstones = {str(x) for x in (canvas.get("deleted_node_ids") or [])}
    if tombstones and canvas.get("nodes"):
        canvas["nodes"] = [n for n in canvas["nodes"] if str(n.get("id")) not in tombstones]
...
async def update_canvas(canvas_id: str, payload: CanvasSaveRequest):
    ...
    # 保存时剔除墓碑节点：旧客户端快照/竞态若把已删节点带回来，这里一并挡掉
    tombstones = {str(x) for x in (canvas.get("deleted_node_ids") or [])}
    if tombstones and payload.nodes:
        canvas["nodes"] = [n for n in payload.nodes if str(n.get("id")) not in tombstones]
    else:
    ...
    canvas["comfyQueue"] = payload.comfyQueue or []
    save_canvas(canvas)
    await manager.broadcast_canvas_updated(canvas_id, int(canvas.get("updated_at") or now_ms()), payload.client_id)
    return {"canvas": canvas}

# --- 画布 MCP 控制：外部 AI 控制画布节点 ---
# 说明：mcp_server.py（官方 mcp SDK）通过 HTTP 调用本文件端点新建/修改节点、读取"用户选中节点"。
# 保存后复用 broadcast_canvas_updated 广播，前端 handleCanvasUpdatedMessage -> 远程重同步，
# 会自动渲染出新节点/更新文本，因此无需改动前端渲染逻辑。
# 鉴权 token：默认内置，可用环境变量 MCP_API_TOKEN 覆盖；mcp_server.py 与本文件共用同一 token。
MCP_API_TOKEN = os.environ.get("MCP_API_TOKEN", "wucanvas-mcp-default-token")

# 前端定时上报的"当前选中节点 + 可视区中心"，供 MCP 定位用户选中的节点、及新建节点默认放置点
MCP_SELECTION = {}  # canvas_id -> {"node_id": str|None, "x": float, "y": float, "at": int}

class McpPromptNodeRequest(BaseModel):
    canvas_id: str
    x: float = 0.0
    y: float = 0.0
    text: str = ""
...
class McpPromptNodeUpdateRequest(BaseModel):
    canvas_id: str
    node_id: str
    text: str = ""
    token: str = ""

@app.post("/api/mcp/prompt-node")
async def create_mcp_prompt_node(payload: McpPromptNodeRequest):
    ...
    # 未显式指定坐标（x/y 均为 0）时，放到前端最近上报的可视区中心
    if not (float(payload.x or 0) or float(payload.y or 0)):
        info = MCP_SELECTION.get(payload.canvas_id) or {}
        node_x = float(info.get("x") or 0)
        node_y = float(info.get("y") or 0)
    else:
        node_x = float(payload.x)
        node_y = float(payload.y)
    node = {
        "id": "prompt_" + uuid.uuid4().hex,
        "type": "prompt",
        "x": node_x,
        "y": node_y,
...
@app.put("/api/canvases/{canvas_id}/mcp-selection")
async def report_mcp_selection(canvas_id: str, payload: McpSelectionRequest):
    """前端上报：当前选中的单个节点 + 可视区中心世界坐标（用于 MCP 定位与新建节点落点）。"""
    MCP_SELECTION[canvas_id] = {
...
@app.get("/api/canvases/{canvas_id}/mcp-selection")
async def get_mcp_selection(canvas_id: str):
    """返回该画布最近上报的选中节点详情与视口中心，供 mcp_server 的 get_selected_node 读取。"""
    info = MCP_SELECTION.get(canvas_id) or {}
    node_id = info.get("node_id")
    node = None
    if node_id:
        canvas = load_canvas(canvas_id)
        node = next((n for n in (canvas.get("nodes") or []) if n.get("id") == node_id), None)
    return {"canvas_id": canvas_id, "node": node, "x": info.get("x"), "y": info.get("y"), "at": info.get("at")}

@app.post("/api/mcp/prompt-node/update")
async def update_mcp_prompt_node(payload: McpPromptNodeUpdateRequest):
    from hmac import compare_digest
    if not compare_digest(str(payload.token or ""), MCP_API_TOKEN):
        raise HTTPException(status_code=401, detail="MCP Token 无效")
    canvas = load_canvas(payload.canvas_id)
    node = next((n for n in (canvas.get("nodes") or []) if n.get("id") == payload.node_id), None)
    if node is None:
        raise HTTPException(status_code=404, detail="节点不存在")
    node["text"] = payload.text or ""
    save_canvas(canvas)
    await manager.broadcast_canvas_updated(payload.canvas_id, int(canvas.get("updated_at") or now_ms()), "mcp")
    return {"node": node, "updated_at": int(canvas.get("updated_at") or now_ms())}
...
class CanvasTombstoneRequest(BaseModel):
    ...
    remove: List[str] = []  # 移除墓碑：撤销删除时恢复这些节点

@app.post("/api/canvases/{canvas_id}/tombstones")
async def update_canvas_tombstones(canvas_id: str, payload: CanvasTombstoneRequest):
    """删除墓碑增量上报：前端删除节点瞬间就调用，弱网下即使主 save 失败，刷新也不会复活已删节点。"""
    canvas = load_canvas(canvas_id)
    tombstones = set(str(x) for x in (canvas.get("deleted_node_ids") or []))
    for x in (payload.add or []):
        tombstones.add(str(x))
    for x in (payload.remove or []):
        tombstones.discard(str(x))
    canvas["deleted_node_ids"] = list(tombstones)
    save_canvas(canvas)
    return {"ok": True, "deleted_node_ids": canvas["deleted_node_ids"]}
...
def generate(req: GenerateRequest, progress_cb=None):
    global NEXT_TASK_ID
    current_task = None
    target_backend = None
    with QUEUE_LOCK:
    ...
    try:
        ...
        def _report(stage):
            try:
                if progress_cb:
                    progress_cb({"stage": stage, "prompt_id": prompt_id, "backend": target_backend})
            except Exception:
                pass
        _report("submitted")

        # 等待云端完成：history 出现即结束；期间用 /queue 三查防"任务丢失空转"（防空转核心）
        history_data = None
        poll_round = 0
        queue_miss = 0
        poll_deadline = time.monotonic() + COMFYUI_HISTORY_TIMEOUT
        while time.monotonic() < poll_deadline:
            res = get_comfy_history(target_backend, prompt_id)
            if prompt_id in res:
                entry = res[prompt_id]
                if str((entry.get("status") or {}).get("status_str") or "").lower() == "error":
                    raise Exception(comfy_execution_error_message(entry, workflow))
                history_data = entry
                break
            poll_round += 1
            # 每 10 轮（约 5s）查一次云端队列，确认任务还在（排队中/执行中）
            if poll_round % 10 == 0:
                try:
                    qstatus = comfy_queue_status(target_backend, prompt_id)
                    if qstatus:
                        queue_miss = 0
                        _report(qstatus)
                    else:
                        queue_miss += 1
                        if queue_miss >= 3:
                            raise Exception("云端 ComfyUI 任务丢失：运行队列和历史记录中都找不到该任务，请检查云端后重试")
                except Exception as e:
                    if "任务丢失" in str(e):
                        raise
                    # 云端暂时不可达：不算丢失（清零计数），由 history 轮询继续兜底
                    queue_miss = 0
            time.sleep(0.5)
...
def generate_api(req: GenerateRequest):
    """薄封装：保留 /api/generate 兼容路由（progress_cb 仅供画布任务线程内部使用，不走 HTTP）"""
    return generate(req)
