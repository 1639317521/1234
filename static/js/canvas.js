...
const NODE_RECT_CACHE = new Map();  // nodeId -> {w,h}：实际渲染尺寸缓存，避免小地图/适配视图逐节点 querySelector+offsetWidth 强制重排
function estimatedNodeRect(n){
    // 性能：优先显式尺寸，其次渲染尺寸缓存，最后默认值——不再逐节点 querySelector+offsetWidth（拖动时每帧 58 次强制重排的根源）
    const size = defaultNodeSize(n.type);
    const cached = NODE_RECT_CACHE.get(n.id);
    const w = Number(n.w) || (cached && cached.w) || size.w || 260;
    const h = Number(n.h) || (cached && cached.h) || size.h || 160;
...
function scheduleMinimapRender(){
    // 性能：节点拖动/缩放/画布平移期间冻结小地图全量重建（每帧 innerHTML 重建是卡顿主因）；
    // 平移时节点边界不变，只需移动视口框；松手后由 endDrag 统一刷新一次
    if(document.body.classList.contains('canvas-node-drag') || document.body.classList.contains('canvas-node-resize')) return;
    if(document.body.classList.contains('canvas-board-pan')){
...
function updateNodeRectCache(ids){
    if(!nodesEl) return;
    const list = (ids && ids.length)
        ? ids.map(id => nodes.find(n => n.id === id)).filter(Boolean)
        : nodes;
    for(const n of list){
        const el = nodesEl.querySelector(`.node[data-id="${CSS.escape(n.id)}"]`);
        if(el) NODE_RECT_CACHE.set(n.id, {w: el.offsetWidth, h: el.offsetHeight});
        else NODE_RECT_CACHE.delete(n.id);
    }
    if(!ids || !ids.length){
        const alive = new Set(nodes.map(n => n.id));
        for(const id of [...NODE_RECT_CACHE.keys()]){
            if(!alive.has(id)) NODE_RECT_CACHE.delete(id);
        }
    }
}
...
function mergeRemoteNewIntoLocal(remote){
    if(!remote || !canvas || remote.id !== canvas.id) return false;
    let added = false;
    const localNodeIds = new Set(nodes.map(n => n.id));
    (remote.nodes || []).forEach(n => {
        if(n && n.id && !localNodeIds.has(n.id)){ nodes.push(n); localNodeIds.add(n.id); added = true; }
    });
    const localConnIds = new Set(connections.map(c => c.id));
    const localConnPairs = new Set(connections.map(c => `${c.from}>${c.to}`));
    (remote.connections || []).forEach(c => {
        if(!c || !c.id) return;
        if(localConnIds.has(c.id) || localConnPairs.has(`${c.from}>${c.to}`)) return;
        if(!localNodeIds.has(c.from) || !localNodeIds.has(c.to)) return;
        connections.push(c);
        localConnIds.add(c.id);
        localConnPairs.add(`${c.from}>${c.to}`);
        added = true;
    });
    if(added) render();
    return added;
    ...
    try {
        ...
        if(res.status === 409){
            ...
            if(localCanvasDirty || saveCanvasAgain){
                mergeRemoteNewIntoLocal(remote);
...
async function createCanvas(){
    ...
    try {
        ...
        canvas.comfyQueue = canvas.comfyQueue || [];
        nodes = canvas.nodes || [];
...
function openSelectionMenu(clientX, clientY){
    ...
    const groupableCount = selectedNodes.filter(node => node.type === 'image' || node.type === 'prompt').length;
    const canGroup = groupableCount >= 2;
    const runnableCount = selectedNodes.filter(node => canvasRunTypes().includes(node.type) && !node.running && !cascadeRunningIds.has(node.id)).length;
    if(!canGroup && !runnableCount) return false;
    closeCreateMenu();
    imageNodeMenu.innerHTML = `
        ${canGroup ? '<button class="menu-btn" data-selection-group><i data-lucide="group" class="w-4 h-4"></i><span>打组</span></button>' : ''}
        ${runnableCount ? `<button class="menu-btn" data-selection-run><i data-lucide="play" class="w-4 h-4"></i><span>批量运行（${runnableCount}）</span></button>` : ''}
    `;
    imageNodeMenu.style.left = `${clientX}px`;
    imageNodeMenu.style.top = `${clientY}px`;
    imageNodeMenu.classList.add('open');
    const groupBtn = imageNodeMenu.querySelector('[data-selection-group]');
    if(groupBtn) groupBtn.onclick = e => {
        e.stopPropagation();
        closeImageNodeMenu();
        groupSelectedImages();
    };
    const runBtn = imageNodeMenu.querySelector('[data-selection-run]');
    if(runBtn) runBtn.onclick = async e => {
        e.stopPropagation();
        closeImageNodeMenu();
        await runSelectedGenerators();
    };
    refreshIcons();
    return true;
}
...
function openOutputNodeMenu(nodeId, clientX, clientY){
    ...
    // 本地缓存文件（/output/ 或 /assets/）才能"打开所在文件夹"，远程 /api/view 等不支持
    const localOutputUrls = (node?.images || []).map(outputUrlValue).filter(url => url && (url.startsWith('/output/') || url.startsWith('/assets/')));
    ...
    const openFolderBtn = imageNodeMenu.querySelector('[data-output-open-folder]');
    if(openFolderBtn){
        openFolderBtn.onclick = async e => {
            e.stopPropagation();
            closeImageNodeMenu();
            const url = localOutputUrls[0];
            if(!url) return;
            try {
                ...
                const data = await res.json();
                if(!res.ok || data.success === false) throw new Error(data.detail || tr('canvas.outputOpenFolderFailed'));
            } catch(err) {
                alert(err.message || tr('canvas.outputOpenFolderFailed'));
            }
        };
    }
    refreshIcons();
...
function createNodeByType(type, point){
    ...
    if(type === 'frame') return addFrameNode(point);
    return null;
}
function createMenuCounts(){
    let counts = {};
    try{ counts = JSON.parse(localStorage.getItem('canvas_create_menu_counts') || '{}') || {}; }catch(e){}
    return counts;
}
function sortCreateMenu(){
    // 新建菜单按使用频率自动排序：越常用的类型排越前
    const menu = document.getElementById('createMenu');
    if(!menu) return;
    const counts = createMenuCounts();
    const btns = [...menu.querySelectorAll('.menu-btn')];
    btns.sort((a, b) => {
        const ta = (a.getAttribute('onclick').match(/menuAdd\('([\w]+)'\)/) || [])[1] || '';
        const tb = (b.getAttribute('onclick').match(/menuAdd\('([\w]+)'\)/) || [])[1] || '';
        return (counts[tb] || 0) - (counts[ta] || 0);
    });
    btns.forEach(b => menu.appendChild(b));
}
function bumpCreateMenu(type){
    const counts = createMenuCounts();
    counts[type] = (counts[type] || 0) + 1;
    try{ localStorage.setItem('canvas_create_menu_counts', JSON.stringify(counts)); }catch(e){}
    sortCreateMenu();
}
function menuAdd(type){
    ...
    if(type === 'frame') addFrameNode(menuPoint);
}
...
function isAudioUrl(url){
    const raw = String(url || '');
    if(/\.(mp3|wav|m4a|aac|ogg|flac)(\?|$)/i.test(canvasOriginalMediaUrl(raw))) return true;
    // 兼容后端 /api/view 代理 URL（?filename=xxx.wav&...）
    try {
        const parsed = new URL(canvasOriginalMediaUrl(raw), window.location.origin);
        return /\.(mp3|wav|m4a|aac|ogg|flac)$/i.test(parsed.searchParams.get('filename') || '');
...
const _progressPendingIds = new Set();
function requestProgressRefresh(ids){
    // 性能：生成进度消息可能高频到达（云端每步/每节点一条），把多个节点的刷新合并到 300ms 窗口内一次执行
    let changed = false;
    for(const id of (ids || [])){
        if(id && !_progressPendingIds.has(id)){ _progressPendingIds.add(id); changed = true; }
    }
    if(!changed || _progressFlushTimer) return;
    _progressFlushTimer = setTimeout(() => {
        _progressFlushTimer = null;
        const pending = [..._progressPendingIds];
        _progressPendingIds.clear();
        if(pending.length) refreshNodes(pending);
    }, 300);
}
function refreshRunNodes(node, out=null){
    requestProgressRefresh([node?.id, out?.id]);
}
...
function renderPendingOutput(pending){
    ...
    const queuedPending = pending.queued || !Number(pending.startedAt) || phase === 'submitted' || phase === 'queued';
    const timeHtml = uploading
        ? `<span class="output-time-pill queued">${tr('canvas.pendingUploading')}</span>`
        : queuedPending
            ? `<span class="output-time-pill queued">${tr('canvas.pendingQueued')}</span>`
            : `<span class="output-time-pill running">${formatRunDuration(nowMs() - Number(pending.startedAt || nowMs()))}</span>`;
    // WS 实时进度（方案 A）：当前节点 / 已执行节点数 / 采样步进
    const pInfo = phase === 'running' ? (pending.progressInfo || null) : null;
    const pInfoParts = [];
    if(pInfo?.nodeTitle) pInfoParts.push(`正在执行「${pInfo.nodeTitle}」`);
    if(pInfo?.sampler?.max) pInfoParts.push(`采样 ${pInfo.sampler.value}/${pInfo.sampler.max}`);
    if(pInfo?.executed && pInfo?.total) pInfoParts.push(`节点 ${pInfo.executed}/${pInfo.total}`);
    const infoHtml = pInfoParts.length
        ? `<div class="output-pending-info" title="${escapeAttr(pInfoParts.join(' · '))}" style="font-size:10px;color:#9ca3af;text-align:center;padding:0 6px 4px;line-height:1.35;word-break:break-all;max-width:100%;box-sizing:border-box;">${escapeHtml(pInfoParts.join(' · '))}</div>`
        : '';
    return `<div class="output-img-wrap loading-wrap" data-pending-id="${escapeAttr(pending.id)}"${pendingOutputStyle(pending)}>${timeHtml}<div class="output-spinner"></div>${infoHtml}<button class="output-del" title="${tr('common.delete')}">×</button></div>`;
}
...
function renderNode(node){
    ...
    el.innerHTML = `<div class="node-head"><span class="node-title" title="双击修改标题">${escapeHtml(displayTitle)}</span><div style="display:flex;align-items:center;gap:8px">${statusHtml}<button onclick="deleteNodeFromButton('${node.id}', event)" class="text-gray-300 hover:text-red-500"><i data-lucide="x" class="w-4 h-4"></i></button></div></div>`;
    const nodeTitleEl = el.querySelector('.node-title');
    if(nodeTitleEl){
        nodeTitleEl.onmousedown = e => e.stopPropagation();
        nodeTitleEl.ondblclick = e => { e.stopPropagation(); e.preventDefault(); renameNodeTitle(node); };
    }
    ...
    if(node.type === 'frame') body.appendChild(renderFrameBody(node));
    ...
    const canInput = ['generator','midjourney','comfy','ltxDirector','output','llm','msgen','video','rh','minimax','frame'].includes(node.type) || (node.type === 'loop' && (node.imageInput || node.showPrompt));
...
function renameNodeTitle(node){
    if(!node) return;
    const el = nodesEl.querySelector(`.node[data-id="${CSS.escape(node.id)}"] .node-title`);
    if(!el) return;
    const input = document.createElement('input');
    ...
    const commit = () => {
        ...
        if(value !== (node.title || '')){
            node.title = value || undefined;
            scheduleSave();
            renderComfyQueuePanel();
        }
        render();
    };
    input.onblur = commit;
...
function defaultNodeSize(type){
    ...
    if(type === 'frame') return {w:380, h:0};
...
// ===== 取帧节点：从上游视频抽取最后一帧/指定时间/指定帧号为图片 =====
function addFrameNode(point){
...
function frameNodeVideoSources(node){
    const urls = [];
    for(const c of connections.filter(cc => cc.to === node.id)){
        const src = nodes.find(n => n.id === c.from);
        if(!src) continue;
        const candidates = [];
        if(Array.isArray(src.images)) src.images.forEach(it => {
            const u = typeof it === 'string' ? it : (it.url || '');
            if(u) candidates.push(u);
        });
        if(src.url) candidates.push(src.url);
        candidates.forEach(u => { if(isVideoUrl(u)) urls.push(u); });
    }
    return [...new Set(urls)];
}
function formatFrameSeconds(sec){
    sec = Math.max(0, Number(sec) || 0);
    const m = Math.floor(sec / 60), s = Math.floor(sec % 60), cs = Math.round((sec - Math.floor(sec)) * 100);
    return `${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}.${String(cs).padStart(2,'0')}`;
}
function loadVideoForFrames(url){
    return new Promise((resolve, reject) => {
        ...
        v.onerror = () => { clearTimeout(timer); reject(new Error('浏览器无法解码该视频（可能是不支持的编码如 HEVC），请转成 H.264 mp4 后重试')); };
    });
}
function seekVideoTo(v, t){
    return new Promise(resolve => {
        const done = () => { v.removeEventListener('seeked', onSeeked); clearTimeout(timer); resolve(); };
        const onSeeked = done;
        const timer = setTimeout(done, 4000);
        v.addEventListener('seeked', onSeeked);
        try{ v.currentTime = Math.max(0, Number(t) || 0); }catch(e){ done(); }
    });
}
async function grabFrameBlob(v){
    const c = document.createElement('canvas');
    c.width = v.videoWidth || 1280; c.height = v.videoHeight || 720;
    c.getContext('2d').drawImage(v, 0, 0, c.width, c.height);
    return await new Promise(res => c.toBlob(res, 'image/png'));
}
function renderFrameBody(node){
    const wrap = document.createElement('div');
    wrap.className = 'generator-body';
    const vids = frameNodeVideoSources(node);
    if(!vids.includes(node.frameSrc)) node.frameSrc = vids[0] || '';
    const mode = ['last','time','index'].includes(node.frameMode) ? node.frameMode : 'last';
    const srcName = node.frameSrc ? (decodeURIComponent(node.frameSrc.split('?')[0]).split('/').pop() || '已连接') : '';
    wrap.innerHTML = `
        <div class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">视频源</div>
        <div class="frame-src-line ${node.frameSrc ? '' : 'frame-empty'}" title="${escapeAttr(node.frameSrc || '')}">${escapeHtml(node.frameSrc ? srcName : '未连接视频源：请从上游连线接入')}</div>
        <div class="frame-preview-wrap"><canvas class="frame-preview"></canvas><span class="frame-preview-hint">加载中…</span></div>
        <div class="gen-settings-row frame-mode-row">
            <button type="button" data-fmode="last" class="setting-check ${mode==='last'?'active':''}">最后一帧</button>
            <button type="button" data-fmode="time" class="setting-check ${mode==='time'?'active':''}">指定时间</button>
            <button type="button" data-fmode="index" class="setting-check ${mode==='index'?'active':''}">指定帧号</button>
        </div>
        <div class="gen-settings-row" data-row="time" ${mode==='time' ? '' : 'hidden'}>
            <input type="range" class="frame-slider" min="0" max="10000" value="${Math.round((Number(node.frameTime)||0)*100)}" style="flex:1 1 auto">
            <span class="frame-time-label" style="font-size:11px;font-weight:800;font-variant-numeric:tabular-nums">${formatFrameSeconds(Number(node.frameTime)||0)}</span>
        </div>
        <div class="gen-settings-row" data-row="index" ${mode==='index' ? '' : 'hidden'}>
            <label style="display:flex;align-items:center;gap:6px;font-size:11px;font-weight:700">帧号<input type="number" class="setting-input frame-idx-input" style="width:90px;height:30px" min="0" value="${Number(node.frameIndex)||0}"></label>
            <label style="display:flex;align-items:center;gap:6px;font-size:11px;font-weight:700">FPS<input type="number" class="setting-input frame-fps-input" style="width:70px;height:30px" min="1" max="120" value="${Number(node.frameFps)||25}"></label>
        </div>
        <div class="gen-run-row">
            <button type="button" class="gen-btn frame-extract-btn ${node.busy ? 'running' : ''}" ${node.busy ? 'disabled' : ''}><i data-lucide="scissors" class="w-4 h-4"></i>${node.busy ? '提取中…' : '提取为图片'}</button>
        </div>`;
    const preview = wrap.querySelector('.frame-preview');
    const hint = wrap.querySelector('.frame-preview-hint');
    let vid = null, dur = 0, previewTimer = 0;
    function targetSeconds(){
        const d = dur || 0;
        if(mode === 'last') return Math.max(0, d - 0.04);
        if(mode === 'index') return (Number(node.frameIndex) || 0) / (Number(node.frameFps) || 25);
        return Number(node.frameTime) || 0;
    }
    async function drawPreview(){
        try{
            if(!node.frameSrc){ hint.textContent = '未连接视频源'; return; }
            if(!vid){ hint.textContent = '加载视频…'; vid = await loadVideoForFrames(node.frameSrc); dur = (isFinite(vid.duration) && vid.duration > 0) ? vid.duration : 0; }
            const t = targetSeconds();
            hint.textContent = '定位中…';
            await seekVideoTo(vid, t);
            preview.width = vid.videoWidth || 356; preview.height = vid.videoHeight || 200;
            preview.getContext('2d').drawImage(vid, 0, 0, preview.width, preview.height);
            hint.textContent = `${preview.width}\u00d7${preview.height} \u00b7 ${formatFrameSeconds(vid.currentTime)}${dur ? ' / ' + formatFrameSeconds(dur) : ''}`;
        }catch(err){
            hint.textContent = err.message || '预览失败';
        }
    }
    function schedulePreview(){
        clearTimeout(previewTimer);
        previewTimer = setTimeout(drawPreview, 160);
    }
    wrap.querySelectorAll('[data-fmode]').forEach(btn => {
        btn.onclick = e => {
            e.stopPropagation();
            node.frameMode = btn.dataset.fmode;
            refreshNodes([node.id]);
            scheduleSave();
        };
    });
    const slider = wrap.querySelector('.frame-slider');
    const timeLabel = wrap.querySelector('.frame-time-label');
    if(slider) slider.oninput = () => {
        const sec = Number(slider.value) / 100;
        node.frameTime = sec;
        if(timeLabel) timeLabel.textContent = formatFrameSeconds(sec);
        schedulePreview();
    };
    const idxInput = wrap.querySelector('.frame-idx-input');
    if(idxInput) idxInput.onchange = () => { node.frameIndex = Math.max(0, Number(idxInput.value) || 0); idxInput.value = node.frameIndex; schedulePreview(); scheduleSave(); };
    const fpsInput = wrap.querySelector('.frame-fps-input');
    if(fpsInput) fpsInput.onchange = () => { node.frameFps = Math.min(120, Math.max(1, Number(fpsInput.value) || 25)); fpsInput.value = node.frameFps; scheduleSave(); };
    setTimeout(drawPreview, 30);
    wrap.querySelector('.frame-extract-btn').onclick = async e => {
        e.stopPropagation();
        const btn = e.currentTarget;
        if(!node.frameSrc){ alert('请先从上游连线接入视频源'); return; }
        btn.disabled = true; const oldHtml = btn.innerHTML; btn.textContent = '提取中…';
        try{
            node.busy = true;
            if(!vid){ vid = await loadVideoForFrames(node.frameSrc); dur = (isFinite(vid.duration) && vid.duration > 0) ? vid.duration : 0; }
            const t = targetSeconds();
            await seekVideoTo(vid, t);
            const blob = await grabFrameBlob(vid);
            if(!blob) throw new Error('截帧失败，请重试');
            const fd = new FormData();
            fd.append('files', blob, `frame_${Date.now()}.png`);
            const res = await fetch('/api/ai/upload', { method:'POST', body: fd });
            if(!res.ok) throw new Error(await responseErrorMessage(res, '上传失败'));
            const up = await res.json();
            const url = up.files?.[0]?.url || '';
            if(!url) throw new Error('上传失败');
            const imgNode = {
                id: uid('img'), type: 'image',
                x: (node.x || 0) + 430, y: (node.y || 0),
                url,
                name: `取帧_${formatFrameSeconds(t).replace(/[:.]/g, '-')}`,
            };
            nodes.push(imgNode);
            selected.clear(); selected.add(imgNode.id);
            render();
            scheduleSave();
            setStatus('已提取并创建图片节点');
        }catch(err){
            alert(err.message || '取帧失败');
        }finally{
            node.busy = false;
...
function renderImageInputList(list, node, imageInputs, emptyText=null){
    ...
    attachGroupExcludeControls(list, node, imageInputs);
    refreshIcons();
}
function attachGroupExcludeControls(list, node, imageInputs){
    // 分组来源排除：通用注入——generator/mj/msgen/comfy/ltxDirector/video/rh 的图片输入列表共用
    // 注意：imageInputs 为空（整组被排除时源头过滤结果为空）也必须继续，否则恢复行永远不渲染
    if(!list || !node) return;
    const inputs = Array.isArray(imageInputs) ? imageInputs : [];
    const excludedListEarly = Array.isArray(node.excludedGroupItems) ? node.excludedGroupItems : [];
    if(!inputs.length && !excludedListEarly.length) return;
    const byId = new Map(inputs.map(s => [s.id, s]));
    [...list.querySelectorAll('.input-item')].forEach(item => {
        const src = byId.get(item.dataset.sourceId);
        if(!src || !src.groupId || !src.imageId || item.querySelector('.input-ex-toggle')) return;
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'input-ex-toggle';
        btn.title = '从本生成器的传递中排除此图（不影响分组和其他生成器）';
        btn.innerHTML = '<i data-lucide="eye-off" class="w-3.5 h-3.5"></i>';
        btn.onclick = e => {
            e.stopPropagation();
            toggleGroupSourceExclude(node, src.groupId, src.imageId);
        };
        item.appendChild(btn);
    });
    const excludedList = Array.isArray(node.excludedGroupItems) ? node.excludedGroupItems : [];
    if(!excludedList.length) return;
    const activeKeys = new Set(inputs.map(s => s.id));
    const groupIds = new Set([
        ...[...activeKeys].map(id => String(id).split(':')[0]),
        ...excludedList.map(key => String(key).split(':')[0]),
    ]);
    const restoredRows = [];
    for(const key of excludedList){
        const gid = String(key).split(':')[0];
        const mid = String(key).split(':').slice(1).join(':');
        if(!groupIds.has(gid) || activeKeys.has(key)) continue;
        const g = nodes.find(n => n.id === gid);
        const member = g && (g.items || []).includes(mid) ? nodes.find(n => n.id === mid) : null;
        if(!member) continue;
        restoredRows.push({gid, mid, label: member.name || (member.text || '').slice(0, 24) || mid});
    }
    if(!restoredRows.length) return;
    const row = document.createElement('div');
    row.className = 'input-item input-item-excluded';
    row.innerHTML = `<span class="input-label" style="flex:1">已排除 ${restoredRows.length} 项（不随本生成器传递）</span><button type="button" class="input-ex-restore">恢复</button>`;
    row.querySelector('.input-ex-restore').onclick = e => {
        e.stopPropagation();
        const removeSet = new Set(restoredRows.map(r => `${r.gid}:${r.mid}`));
        node.excludedGroupItems = excludedList.filter(k => !removeSet.has(k));
        scheduleSave();
        refreshNodes([node.id]);
    };
    list.appendChild(row);
}
function toggleGroupSourceExclude(node, groupId, memberId){
    const key = `${groupId}:${memberId}`;
    const cur = Array.isArray(node.excludedGroupItems) ? [...node.excludedGroupItems] : [];
    const at = cur.indexOf(key);
    if(at >= 0) cur.splice(at, 1); else cur.push(key);
    node.excludedGroupItems = cur;
    scheduleSave();
    refreshNodes([node.id]);
}
...
function renderComfyBody(node){
    ...
    const nodeQueued = comfyQueueItems().some(q => q.nodeId === node.id && q.status === 'queued');
    ...
    wrap.innerHTML = `
        ...
        <div class="comfy-controls">
            ...
            <div class="gen-run-row">
                ...
                <button class="comfy-enqueue ${nodeQueued ? 'active' : ''}" type="button" onclick="enqueueComfyNode('${node.id}')" title="${nodeQueued ? '该节点已在生成队列中' : '加入生成等待队列，稍后批量提交'}" aria-label="加入生成队列"><i data-lucide="${nodeQueued ? 'clock' : 'list-plus'}" class="w-4 h-4"></i>${nodeQueued ? '排队中' : '排队'}</button>
                ${cascadeBtnHtml(node)}
            </div>
            ${retryBarHtml(node)}
        </div>
...
function rhReadableError(data, fallback='RunningHub 失败'){
    ...
    if(value && typeof value === 'object'){
        ...
        if(inner){
            const raw = value.raw;
            const rawMsg = raw && typeof raw === 'object'
                ? [raw.message, raw.msg, raw.error, raw.detail, raw.reason].find(v => typeof v === 'string' && v && v !== inner)
                : null;
            return rawMsg ? `${inner}（${rawMsg}）` : inner;
        }
        try {
...
function renderRhInputs(list, node, media){
    ...
    attachGroupExcludeControls(list, node, media.sources);
    refreshIcons();
}
...
async function rhFetchAppInfo(nodeId, showAlert=true){
    ...
    try {
        ...
        const data = await res.json();
        if(!res.ok || data.success === false) throw new Error(rhReadableError(data, tr('canvas.rhFailed')));
...
async function rhUploadValueIfNeeded(value, node=null){
    ...
    const data = await res.json();
    if(!res.ok || data.success === false) throw new Error(rhReadableError(data, tr('canvas.rhUploadFailed')));
...
async function runRhNode(nodeId, opts={}){
    ...
    try {
        ...
        const submit = await cascadeFetch(endpoint, {
            ...
            if(!r.ok || data.success === false) throw new Error(rhReadableError(data, tr('canvas.rhFailed')));
        ...
        for(let i = 0; i < 720; i++){
            ...
            const data = await cascadeFetch(`/api/runninghub/query?taskId=${encodeURIComponent(taskId)}&useWallet=${useWallet ? '1' : '0'}`, {}, {cascadeTargetId}).then(async r => {
                ...
                if(!r.ok || json.success === false) throw new Error(rhReadableError(json, tr('canvas.rhFailed')));
...
function generatedImageRefs(node){
    ...
    return (node?.generatedOutputs || [])
        .map((item, i) => {
            ...
            if(!url) return null;
            const kind = mediaKindForOutputItem(item);
            const nm = (item && typeof item === 'object' && item.name) || outputImageName(url) || `${node.type || 'generated'}-${i + 1}`;
return {url, name:nm, kind, index:i};
        })
        .filter(Boolean)
...
function generatorSources(gen){
    return connections.filter(c => c.to === gen.id).map(c => nodes.find(n => n.id === c.from)).filter(Boolean).map(n => {
        ...
        if(n.type === 'group') {
            const __excludedSet = new Set(Array.isArray(gen?.excludedGroupItems) ? gen.excludedGroupItems : []);
            const items = (n.items || []).map(id => nodes.find(x => x.id === id)).filter(Boolean)
                .filter(x => !__excludedSet.has(`${n.id}:${x.id}`));
...
async function collectComfyRunParams(node){
    if(!node) return null;
    const sources = orderedSources(node, generatorSources(node));
    const prompt = sources.map(s => s.prompt).filter(Boolean).join('\n\n');
    const allRefs = sources.flatMap(s => s.refs || []);
    ...
    if((mode === 'text' || (mode === 'custom' && customPromptFields.length)) && !prompt){ alert(tr('canvas.needPrompt')); return null; }
    // 仅 text/enhance/edit 需要强制输入；custom 模式缺图的槽位自动禁用（提交时 null → 后端删除该输入），不再拦截
    if(mode !== 'text' && mode !== 'custom' && !refs.length){ alert(tr('canvas.needImage')); return null; }
    if(mode === 'custom'){
        ...
        fields.filter(f => comfyFieldKind(f) === 'setting' && comfyRandomEnabled(f) && comfyRandomActive(node, f.id)).forEach(f => {
            node.comfyParams = node.comfyParams || {};
    ...
    const snap = {
        ...
        refs,
        allRefs,
        requestSize:mode === 'text' ? {width:Number(node.width || 1024), height:Number(node.height || 1024)} : null,
        width:Number(node.width || 1024),
        height:Number(node.height || 1024),
        enhanceStrength:Number(node.enhanceStrength ?? 0.5),
        enhanceUpscale:Boolean(node.enhanceUpscale),
        enhanceUpscaleRes:Number(node.enhanceUpscaleRes || 2048),
        editUpscale:Boolean(node.editUpscale),
        editUpscaleRes:Number(node.editUpscaleRes || 2048),
        editSeed:Math.floor(Math.random() * 1000000),
        comfyWorkflow:mode === 'custom' ? (validComfyWorkflowName(node.comfyWorkflow) || comfyWorkflows[0]?.name || '') : '',
        comfyParams:JSON.parse(JSON.stringify(node.comfyParams || {}))
    };
    return snap;
}
async function executeComfyRun(node, snap, opts={}){
    const cascadeTargetId = cascadeTargetIdFromOptions(opts);
    const prompt = snap?.prompt || '';
    const refs = snap?.refs || [];
    const allRefs = snap?.allRefs || refs;
    const mode = snap?.mode || 'text';
    const requestSize = snap?.requestSize || null;
    let out = outputForNode(node, 480);
    const pendingId = opts.pendingId || uid('p');
    const run = runSnapshot(node, prompt, refs);
    run.taskLabel = comfyRunLabel(node);
    if(out){
        const existingPending = (out._pending || []).find(p => p.id === pendingId);
        if(existingPending){
            existingPending.run = run;
            existingPending.startedAt = nowMs();
            existingPending.queued = false;
            existingPending.phase = 'uploading';
        }
        else out._pending = [...(out._pending||[]), makePendingForRun(pendingId, run, node, {refs, requestSize, cascadeTargetId})];
        const justCreated = !existingPending;
        const pendingRef = justCreated ? (out._pending || []).find(p => p.id === pendingId) : existingPending;
        if(pendingRef) pendingRef.phase = 'uploading';
    }
    // 云端阶段回调：submitted/queued → 排队中；running → 生成中（计时从此刻起）；
    // WS 实时进度（方案 A）：当前节点/已执行节点/采样进度
    const comfyProgressOptions = (out) => ({
        cascadeTargetId,
        onProgress: data => {
            const pend = pendingById(out, pendingId);
            const stage = String(data?.stage || '');
            if(stage === 'running'){
                if(pend){
                    const wasRunning = pend.phase === 'running';
                    pend.phase = 'running';
                    pend.queued = false;
                    // 只在进入 running 的那一刻开始计时；后续轮询重复收到 running 不再重置
                    if(!wasRunning) pend.startedAt = nowMs();
                }
            } else if(stage === 'queued' || stage === 'submitted'){
                if(pend){ pend.phase = stage === 'queued' ? 'queued' : 'submitted'; pend.queued = true; }
            }
            const info = {};
            const nodeTitle = String(data?.current_node_title || '').trim();
            const executed = Number(data?.executed_nodes) || 0;
            const total = Number(data?.total_nodes) || 0;
            const sampler = data?.sampler && (data.sampler.value !== undefined) ? {value:Number(data.sampler.value), max:Number(data.sampler.max)} : null;
            if(nodeTitle) info.nodeTitle = nodeTitle;
            if(executed) info.executed = executed;
            if(total) info.total = total;
            if(sampler && sampler.max) info.sampler = sampler;
            if(Object.keys(info).length && pend) pend.progressInfo = {...(pend.progressInfo || {}), ...info};
            // 透传给外部（生成队列项等）：阶段 + 实时进度
            if(typeof opts.onStage === 'function'){
                try { opts.onStage({stage, current_node_title:nodeTitle, executed_nodes:executed, total_nodes:total, sampler}); } catch(e) {}
            }
            refreshRunNodes(node, out);
        }
    });
    ...
    try {
        ...
        if(mode === 'text'){
            ...
            const enhance = await runQueuedComfyGenerate({
                ...
                params:{
                    "15": { image:inputName },
                    "204": { value:Number(snap.enhanceStrength ?? 0.5) }
                },
                type:'enhance',
                client_id:CLIENT_ID
            }, comfyProgressOptions(out));
            ...
            if(snap.enhanceUpscale){
                images = await runComfyUpscale(enhance.images?.[0], snap.enhanceUpscaleRes || 2048, {cascadeTargetId});
            } else {
                images = enhance.images || [];
            }
        } else if(mode === 'custom'){
            const workflowName = validComfyWorkflowName(snap.comfyWorkflow || comfyWorkflows[0]?.name || '');
            ...
            settingFields.forEach(f => {
                if(!f.node || !f.input) return;
                params[f.node] = params[f.node] || {};
                const saved = snap.comfyParams?.[f.id];
                params[f.node][f.input] = saved !== undefined ? saved : comfyParamValue(node, f);
            });
            const result = await runQueuedComfyGenerate({
                prompt,
            ...
            const result = await runQueuedComfyGenerate({
                ...
                params:{
                    "168": { text:prompt },
                    "158": { noise_seed:Number(snap.editSeed) || Math.floor(Math.random() * 1000000) },
                    "278": { image:names[0] || "" },
                    "270": { image:names[1] || "" },
                    "292": { image:names[2] || "" },
            ...
            images = snap.editUpscale ? await runComfyUpscale(result.images?.[0], snap.editUpscaleRes || 2048, {cascadeTargetId}) : result.images || [];
        }
        const meta = collectRunMeta(out, pendingId);
        if(out) out._pending = (out._pending||[]).filter(p => p.id !== pendingId);
        ...
        refreshRunNodes(node, out);
        if(opts.cascade) throw err;
        if(!opts.silent) alert(err.message || actionFailed('canvas.comfyGenerate'));
        return {ok:false, error:err.message || String(err)};
    }
}
async function runComfyNode(nodeId, opts={}){
    const node = nodes.find(n => n.id === nodeId);
    if(!node || (node.running && !opts.cascade)) return;
    const snap = await collectComfyRunParams(node);
    if(!snap) return;
    return executeComfyRun(node, snap, opts);
}
// ===== ComfyUI 生成等待队列（先排队，等 GPU 就绪后批量提交）=====
function comfyQueueItems(){
    if(!canvas || !Array.isArray(canvas.comfyQueue)) return [];
    return canvas.comfyQueue;
}
function ensureComfyQueue(){
    if(!canvas) return null;
    if(!Array.isArray(canvas.comfyQueue)) canvas.comfyQueue = [];
    return canvas.comfyQueue;
}
function comfyQueueQueuedCount(){
    return comfyQueueItems().filter(item => item.status === 'queued').length;
}
function updateComfyQueueBadge(){
    const badge = document.getElementById('comfyQueueBadge');
    if(!badge) return;
    const count = comfyQueueQueuedCount();
    badge.textContent = String(count);
    badge.hidden = count === 0;
}
function purgeComfyQueuePending(pendingIds){
    const ids = new Set((pendingIds || []).filter(Boolean));
    if(!ids.size) return;
    nodes.filter(n => n.type === 'output').forEach(out => {
        const before = (out._pending || []).length;
        out._pending = (out._pending || []).filter(p => !ids.has(p.id));
        if((out._pending || []).length !== before) refreshNodes([out.id]);
    });
}
function resetComfyNodeQueueStatus(node){
    if(!node || node.type !== 'comfy' || node.runStatus !== 'queued') return;
    const stillQueued = comfyQueueItems().some(q => q.nodeId === node.id && q.status === 'queued');
    if(!stillQueued){
        node.runStatus = '';
        node.runError = '';
    }
...
function removeComfyQueueItem(id){
    const queue = ensureComfyQueue();
    if(!queue) return;
    const idx = queue.findIndex(item => item.id === id);
    if(idx < 0) return;
    const item = queue[idx];
    queue.splice(idx, 1);
    if(item?.pendingId) purgeComfyQueuePending([item.pendingId]);
    if(item?.nodeId) resetComfyNodeQueueStatus(nodes.find(n => n.id === item.nodeId));
    scheduleSave();
    updateComfyQueueBadge();
    renderComfyQueuePanel();
}
function clearComfyQueueFinished(){
    const queue = ensureComfyQueue();
    if(!queue) return;
    const before = queue.length;
    // 只清「已完成/失败」，保留排队中、提交中、生成中的项
    canvas.comfyQueue = queue.filter(item => item.status === 'queued' || item.status === 'submitting' || item.status === 'running');
    if(canvas.comfyQueue.length !== before){
        scheduleSave();
        updateComfyQueueBadge();
        renderComfyQueuePanel();
    }
}
function purgeComfyQueueForNode(nodeId){
    const queue = ensureComfyQueue();
    if(!queue) return;
    const before = queue.length;
    const removed = queue.filter(item => item.nodeId === nodeId);
    canvas.comfyQueue = queue.filter(item => item.nodeId !== nodeId);
    if(removed.length) purgeComfyQueuePending(removed.map(item => item.pendingId));
    if(canvas.comfyQueue.length !== before){
        scheduleSave();
        updateComfyQueueBadge();
        renderComfyQueuePanel();
...
async function enqueueComfyNode(nodeId){
    if(!canvas) return;
    const node = nodes.find(n => n.id === nodeId);
    if(!node || node.type !== 'comfy') return;
    if(node.running){ alert('当前节点正在运行'); return; }
    const snap = await collectComfyRunParams(node);
    if(!snap) return;
    const queue = ensureComfyQueue();
    if(!queue) return;
    const seq = queue.reduce((m, i) => Math.max(m, Number(i.seq) || 0), 0) + 1;
    const out = outputForNode(node, 480);
    const pendingId = uid('p');
    const run = runSnapshot(node, snap.prompt, snap.refs);
    run.taskLabel = comfyRunLabel(node);
    if(out) out._pending = [...(out._pending||[]), makePendingForRun(pendingId, run, node, {refs:snap.refs, requestSize:snap.requestSize}, {startedAt:0, queued:true})];
    queue.push({
        id:uid('cq'),
        nodeId,
        pendingId,
        ...
        prompt:snap.prompt,
        refs:snap.refs,
        allRefs:snap.allRefs,
        requestSize:snap.requestSize,
        width:snap.width,
        height:snap.height,
        enhanceStrength:snap.enhanceStrength,
        enhanceUpscale:snap.enhanceUpscale,
        enhanceUpscaleRes:snap.enhanceUpscaleRes,
        editUpscale:snap.editUpscale,
        editUpscaleRes:snap.editUpscaleRes,
        editSeed:snap.editSeed,
        comfyWorkflow:snap.comfyWorkflow,
        comfyParams:snap.comfyParams
    });
...
async function submitComfyQueueItem(id){
    const item = comfyQueueItems().find(q => q.id === id);
    if(!item || item.status !== 'queued') return;
    await submitComfyQueueItemInternal(item);
}
async function submitAllComfyQueue(){
    const items = comfyQueueItems().filter(q => q.status === 'queued');
    if(!items.length){ setStatus('队列中没有待提交任务'); return; }
    for(const item of items){
        await submitComfyQueueItemInternal(item);
    }
    setStatus('队列提交完成');
}
async function submitComfyQueueItemInternal(item){
    const node = nodes.find(n => n.id === item.nodeId);
    if(!node || node.type !== 'comfy'){
        item.status = 'failed';
        item.error = '对应节点不存在（可能已删除）';
        renderComfyQueuePanel();
        scheduleSave();
        updateComfyQueueBadge();
        return;
    ...
    const snap = {
        ...
        prompt:item.prompt,
        refs:item.refs || [],
        allRefs:item.allRefs || item.refs || [],
        requestSize:item.requestSize,
        width:item.width,
        height:item.height,
        enhanceStrength:item.enhanceStrength,
        enhanceUpscale:item.enhanceUpscale,
        enhanceUpscaleRes:item.enhanceUpscaleRes,
        editUpscale:item.editUpscale,
        editUpscaleRes:item.editUpscaleRes,
        editSeed:item.editSeed,
        comfyWorkflow:item.comfyWorkflow,
        comfyParams:item.comfyParams
    };
    let result;
    try {
        result = await executeComfyRun(node, snap, {
            pendingId:item.pendingId,
            silent:true,
            // 队列项实时状态：排队中 → 生成中（含当前节点/采样/节点计数）
            onStage: info => {
                const st = String(info?.stage || '');
                if(st === 'running'){
                    item.status = 'running';
                    const pi = {};
                    if(info.current_node_title) pi.nodeTitle = String(info.current_node_title);
                    if(Number(info.executed_nodes)) pi.executed = Number(info.executed_nodes);
                    if(Number(info.total_nodes)) pi.total = Number(info.total_nodes);
                    if(info.sampler?.max) pi.sampler = {value:Number(info.sampler.value), max:Number(info.sampler.max)};
                    item.progressInfo = {...(item.progressInfo || {}), ...pi};
                } else if(st === 'queued' || st === 'submitted'){
                    item.status = 'queued';
                    item.error = '';
                }
                renderComfyQueuePanel();
    ...
    if(result?.ok){
        // 完成后保留在队列里（状态 done），不再自动移除；由「清理已完成」按钮手动清除
        item.status = 'done';
        item.error = '';
        ...
        item.status = 'failed';
        item.error = result?.error || '运行失败';
        renderComfyQueuePanel();
        scheduleSave();
    }
    updateComfyQueueBadge();
}
function toggleComfyQueuePanel(event){
    event?.preventDefault?.();
    event?.stopPropagation?.();
    const panel = document.getElementById('comfyQueuePanel');
    if(!panel) return;
    panel.classList.toggle('open');
    if(panel.classList.contains('open')) renderComfyQueuePanel();
}
function closeComfyQueuePanel(){
    const panel = document.getElementById('comfyQueuePanel');
    panel?.classList.remove('open');
}
function renderComfyQueuePanel(){
    const list = document.getElementById('comfyQueueList');
    if(!list) return;
    const items = comfyQueueItems();
    if(!items.length){
        list.innerHTML = `<div class="comfy-queue-empty">队列为空。在 ComfyUI 节点上点「排队」把任务加入，等 GPU 就绪后一键批量提交。</div>`;
        return;
    }
    const queued = items.filter(item => item.status === 'queued').length;
    list.innerHTML = items.map(item => {
        const node = nodes.find(n => n.id === item.nodeId);
        const base = node ? (String(node.title || '').trim() || comfyRunLabel(node)) : '已删除节点';
        const title = `${base} #${item.seq || '?'}`;
        const modeLabel = ({text:'文生图', enhance:'增强', edit:'编辑'})[item.mode] || item.comfyWorkflow || item.mode || '自定义';
        const statusText = ({queued:'排队中', submitting:'提交中…', running:'生成中', done:'已完成', failed:'失败'})[item.status] || item.status;
        const time = formatQueueTime(item.createdAt);
        const disabled = item.status !== 'queued';
        // 实时进度（WS）：当前节点/采样/节点计数
        const pInfo = item.status === 'running' ? (item.progressInfo || null) : null;
        const pInfoParts = [];
        if(pInfo?.nodeTitle) pInfoParts.push(`正在执行「${pInfo.nodeTitle}」`);
        if(pInfo?.sampler?.max) pInfoParts.push(`采样 ${pInfo.sampler.value}/${pInfo.sampler.max}`);
        if(pInfo?.executed && pInfo?.total) pInfoParts.push(`节点 ${pInfo.executed}/${pInfo.total}`);
        const progressHtml = pInfoParts.length ? `<div class="comfy-queue-item-progress" style="font-size:11px;color:#9ca3af;margin-top:3px;line-height:1.4;word-break:break-all;">${escapeHtml(pInfoParts.join(' · '))}</div>` : '';
        return `<div class="comfy-queue-item ${item.status === 'failed' ? 'failed' : ''} ${item.status === 'running' ? 'running' : ''}">
            <div class="comfy-queue-item-main">
                <div class="comfy-queue-item-title">${escapeHtml(title)}</div>
                <div class="comfy-queue-item-meta">${escapeHtml(modeLabel)} · ${escapeHtml(time)} · <span class="cq-status cq-${escapeAttr(item.status)}">${escapeHtml(statusText)}</span></div>
                ${progressHtml}
                ${item.error ? `<div class="comfy-queue-item-error">${escapeHtml(item.error)}</div>` : ''}
            </div>
            <div class="comfy-queue-item-actions">
                <button type="button" class="cq-btn icon" onclick="locateCanvasNode('${item.nodeId}')" title="定位到画布中的节点"><i data-lucide="crosshair" class="w-3.5 h-3.5"></i>定位</button>
                <button type="button" class="cq-btn primary" ${disabled ? 'disabled' : ''} onclick="submitComfyQueueItem('${item.id}')">提交</button>
                <button type="button" class="cq-btn" onclick="removeComfyQueueItem('${item.id}')">移除</button>
            </div>
        </div>`;
    }).join('') + (queued ? `<div class="comfy-queue-actions"><button type="button" class="cq-btn primary wide" onclick="submitAllComfyQueue()">提交全部（${queued}）</button><button type="button" class="cq-btn" onclick="clearComfyQueueFinished()">清理已完成</button></div>` : '');
    refreshIcons();
}
function locateCanvasNode(nodeId){
    const node = nodes.find(n => n.id === nodeId);
    if(!node) return;
    const rect = board.getBoundingClientRect();
    const r = nodeRect(node);
    viewport = {...viewport, x:rect.width / 2 - (r.x + r.w / 2) * viewport.scale, y:rect.height / 2 - (r.y + r.h / 2) * viewport.scale};
    selected.clear();
    selected.add(nodeId);
    applyViewport();
    refreshSelectionVisuals();
    scheduleViewportSave();
    closeComfyQueuePanel();
}
...
window.toggleComfyQueuePanel = toggleComfyQueuePanel;
window.closeComfyQueuePanel = closeComfyQueuePanel;
window.enqueueComfyNode = enqueueComfyNode;
window.submitComfyQueueItem = submitComfyQueueItem;
window.submitAllComfyQueue = submitAllComfyQueue;
window.removeComfyQueueItem = removeComfyQueueItem;
window.clearComfyQueueFinished = clearComfyQueueFinished;
window.locateCanvasNode = locateCanvasNode;
...
async function runSelectedGenerators(){
    const runTypes = canvasRunTypes();
    const targetIds = [...selected]
        .map(id => nodes.find(node => node.id === id))
        .filter(node => node && runTypes.includes(node.type) && !node.running && !cascadeRunningIds.has(node.id))
        .map(node => node.id);
    for(const nodeId of targetIds){
        try {
            await runCanvasGenerate(nodeId);
        } catch(err) {
            console.error('批量运行节点失败', nodeId, err);
        }
    }
}
...
function isVideoUrl(url){
    const raw = String(url || '');
    const clean = canvasOriginalMediaUrl(raw).split('?')[0].toLowerCase();
    if(/\.(mp4|webm|mov|m4v|avi|mkv|flv)$/.test(clean)) return true;
    // 兼容后端 /api/view 代理 URL（?filename=xxx.mp4&subfolder=...）：扩展名在查询串里
    try {
        const parsed = new URL(canvasOriginalMediaUrl(raw), window.location.origin);
        return /\.(mp4|webm|mov|m4v|avi|mkv|flv)$/i.test(parsed.searchParams.get('filename') || '');
    } catch(e) { return false; }
}
...
function mergeGeneratedOutputs(node, outputs, append=false){
    if(!node) return;
    const keepGeneratedMedia = ['rh','ltxDirector','video','minimax'].includes(node.type);
    const genTitle = String(node.title || '').trim().slice(0, 40);
    let seq = (node.generatedOutputs || []).length;
    const clean = (outputs || []).map(item => {
        ...
        seq += 1;
        if(!genTitle) return kind === 'image' ? url : {url, kind};
        return {url, kind, name:`${genTitle}-${seq}`};
    }).filter(Boolean);
    if(!append){
...
function startSelection(e){
    ...
    selectDrag = {sx:e.clientX, sy:e.clientY, x:e.clientX, y:e.clientY, additive:Boolean(e.ctrlKey || e.metaKey), initialSelected:new Set(selected)};
...
function finishSelection(){
    ...
    const nextSelected = selectDrag.additive ? new Set(selectDrag.initialSelected) : new Set();
    nodesEl.querySelectorAll('.node').forEach(el => {
        const r = el.getBoundingClientRect();
        const overlaps = r.left < rect.right && r.right > rect.left && r.top < rect.bottom && r.bottom > rect.top;
        if(overlaps) nextSelected.add(el.dataset.id);
    });
    selected.clear();
    nextSelected.forEach(id => selected.add(id));
    selectDrag = null;
...
function onNodeDrag(e){
    ...
    // 性能：拖动中仅用 transform 合成位移（GPU 合成、零强制重排），松手由 endDrag 写回 left/top
    ...
    (dragNode.children || []).forEach(childDrag => {
        ...
        if(childEl){
            childEl.style.transform = `translate(${dx}px, ${dy}px)`;
        }
    });
    scheduleLinksRender();
    renderSelectionHub();
...
function canConnect(fromId, toId){
    ...
    // 取帧节点是纯媒体消费者：任何可能携带视频的节点都能连入（frame 端自行过滤非视频源）
    if(to.type === 'frame') return !wouldCreateGeneratorCycle(fromId, toId);
...
function endDrag(event=null){
    ...
    if(dragNode){
        ...
        // 拖动单个图片节点到右侧资产库面板上松手 → 存入资产库并回滚节点位置（不移动画布节点）
        if(event && moved.length === 1 && moved[0].type === 'image' && moved[0].url && !event.shiftKey){
            try{
                const panel = document.getElementById('canvasAssetPanel');
                if(panel){
                    const r = panel.getBoundingClientRect();
                    if(event.clientX >= r.left && event.clientX <= r.right && event.clientY >= r.top && event.clientY <= r.bottom){
                        const ox = Number(dragNode.ox), oy = Number(dragNode.oy);
                        addUrlToCanvasAssetLibrary(moved[0].url, moved[0].name || outputImageName(moved[0].url));
                        moved[0].x = ox; moved[0].y = oy;
                        refreshNodes([moved[0].id]);
                        setStatus('已保存到资产库');
                    }
                }
            }catch(err){ console.error('[canvas] 拖图入库失败', err); }
        }
        ...
        // 拖动中位移走的是 transform：松手把最终坐标写回 left/top 并清除 transform
        moved.forEach(nd => {
            const el2 = nodesEl.querySelector(`.node[data-id="${CSS.escape(nd.id)}"]`);
            if(el2){
                el2.style.left = `${Number(nd.x) || 0}px`;
                el2.style.top = `${Number(nd.y) || 0}px`;
                el2.style.transform = '';
            }
        });
    }
...
function startBoardPan(e, opts={}){
    ...
    closeCreateMenu();
    if(document.activeElement && document.activeElement !== document.body) document.activeElement.blur();
    dragBoard = {sx:e.clientX, sy:e.clientY, ox:viewport.x, oy:viewport.y, moved:false, clearSelectionOnClick:Boolean(opts.clearSelectionOnClick), frame:0, px:e.clientX, py:e.clientY};
    document.body.classList.add('canvas-board-pan');
    // 性能：rAF 合帧，高回报率鼠标一帧多次 mousemove 只应用最后一次位置
    const applyPanFrame = () => {
        if(!dragBoard) return;
        dragBoard.frame = 0;
        viewport.x = dragBoard.ox + dragBoard.px - dragBoard.sx;
        viewport.y = dragBoard.oy + dragBoard.py - dragBoard.sy;
        applyViewport();
    };
    window.onmousemove = e2 => {
        if(Math.hypot(e2.clientX - dragBoard.sx, e2.clientY - dragBoard.sy) > 4) dragBoard.moved = true;
        dragBoard.px = e2.clientX;
        dragBoard.py = e2.clientY;
        if(!dragBoard.frame) dragBoard.frame = requestAnimationFrame(applyPanFrame);
    };
    window.onmouseup = e2 => {
        if(dragBoard?.frame){ cancelAnimationFrame(dragBoard.frame); dragBoard.frame = 0; }
        if(dragBoard){
...
// ===== MCP 选中 + 视口中心上报（外部 AI 定位"当前选中节点"与新建节点落点） =====
// 每 1 秒检查：选中节点变化、可视区中心明显移动、或距上次上报超过 5 秒，任一成立即上报。
// 上报失败静默，不打断画布操作；仅新增独立逻辑，不影响现有渲染。
if (!window.__mcpSelectionReportInstalled) {
    window.__mcpSelectionReportInstalled = true;
    var __mcpCache = { nodeId: '', x: NaN, y: NaN, t: 0 };
    var __COORD_EPS = 60;   // 中心坐标变化超过该值视为明显移动
    var __STALE_MS = 5000;  // 达到该间隔强制兜底上报一次
    var __POLL_MS = 1000;   // 轮询间隔
    function __mcpReport() {
        if (!canvas || !canvas.id) return;
        var sel = selected;
        var nodeId = '';
        if (sel && typeof sel.size === 'number') {
            if (sel.size === 1) nodeId = Array.from(sel)[0];
        }
        var cx = 0, cy = 0;
        try { var c = defaultPoint(0, 0); cx = c.x || 0; cy = c.y || 0; } catch (e) {}
        var now = Date.now();
        var nodeChanged = nodeId !== __mcpCache.nodeId;
        var centerMoved = Math.abs(cx - __mcpCache.x) > __COORD_EPS || Math.abs(cy - __mcpCache.y) > __COORD_EPS;
        var stale = (now - __mcpCache.t) > __STALE_MS;
        if (!nodeChanged && !centerMoved && !stale) return;
        __mcpCache.nodeId = nodeId;
        __mcpCache.x = cx;
        __mcpCache.y = cy;
        __mcpCache.t = now;
        fetch('/api/canvases/' + encodeURIComponent(canvas.id) + '/mcp-selection', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ node_id: nodeId, x: cx, y: cy })
        }).catch(function () {});
    }
    (function __mcpTick() {
        try { __mcpReport(); } catch (e) {}
        setTimeout(__mcpTick, __POLL_MS);
    })();
}
...
// ===== FPS 帧率显示（性能诊断开关）=====
(function(){
    if(window.__canvasFpsMeterInstalled) return;
    window.__canvasFpsMeterInstalled = true;
    const KEY = 'canvas_fps_meter';
    let rafId = 0, frames = 0, lastT = 0, badgeEl = null;

    function ensureBadge(){
        if(badgeEl && document.contains(badgeEl)) return badgeEl;
        badgeEl = document.getElementById('fpsMeterBadge');
        if(!badgeEl){
            badgeEl = document.createElement('div');
            badgeEl.id = 'fpsMeterBadge';
    ...
    function loop(t){
        ...
        if(lastT && t - lastT >= 500){
            ...
            b.style.background = fps < 30 ? 'rgba(220,38,38,.92)' : (fps < 45 ? 'rgba(217,119,6,.92)' : 'rgba(22,101,52,.92)');
        }
        rafId = requestAnimationFrame(loop);
    }
    function stop(){
        if(rafId) cancelAnimationFrame(rafId);
        rafId = 0; frames = 0; lastT = 0;
        const b = ensureBadge(); b.hidden = true;
    }
    window.toggleCanvasFps = function(force){
        const btn = document.getElementById('fpsToggleBtn');
        const on = force === undefined ? !(btn && btn.classList.contains('active')) : !!force;
        try{ localStorage.setItem(KEY, on ? '1' : '0'); }catch(e){}
        if(btn) btn.classList.toggle('active', on);
        if(on && !rafId){ frames = 0; lastT = 0; ensureBadge().hidden = false; rafId = requestAnimationFrame(loop); }
        else if(!on && rafId) stop();
    };
    function boot(){
        let on = false;
        try{ on = localStorage.getItem(KEY) === '1'; }catch(e){}
        if(on) window.toggleCanvasFps(true);
    }
