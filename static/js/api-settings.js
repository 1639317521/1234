...
function syncOnboardingKeyInput(kind, value){
    if(kind === 'free'){
        if(!rhFreeKeyRows.length) rhFreeKeyRows = [{key:'', note:''}];
        if(rhFreeKeyRows[0]){ rhFreeKeyRows[0].key = value || ''; renderRhKeysList('free'); }
    }
    else if(kind === 'wallet'){
        if(!rhWalletKeyRows.length) rhWalletKeyRows = [{key:'', note:'', placeholder:''}];
        if(rhWalletKeyRows[0]) { rhWalletKeyRows[0].key = value || ''; renderRhWalletKeys(); }
    }
...
function syncEditor(){
    ...
    if(item.id === 'runninghub'){
        const freeKeys = collectRhApiKeys('free');
        if(freeKeys.length){
            item.free_keys = freeKeys;
            item.api_key = freeKeys[0].key;
        }
        const walletKeys = collectRhApiKeys('wallet');
        if(walletKeys.length) item.wallet_keys = walletKeys;
        const firstWalletKey = walletKeys[0]?.key || '';
        if(firstWalletKey) item.wallet_api_key = firstWalletKey;
    }
...
function renderEditor(){
    ...
    if(isRunningHub){
        ...
        rhWalletKeyRows = (item.wallet_keys || []).map(it => ({key: it.key || '', note: it.note || '', today_success: Number(it.today_success) || 0, is_next: it.is_next === true, enabled: it.enabled !== false, status: String(it.status || ''), error: String(it.error || ''), status_at: Number(it.status_at) || 0}));
        rhFreeKeyRows = (item.free_keys || []).map(it => ({key: it.key || '', note: it.note || '', today_success: Number(it.today_success) || 0, is_next: it.is_next === true, enabled: it.enabled !== false, status: String(it.status || ''), error: String(it.error || ''), status_at: Number(it.status_at) || 0}));
        renderRhKeysList('wallet');
        renderRhKeysList('free');
        renderRunningHubCards();
    }
    if(isVolcengine){
...
let rhWalletKeyRows = [];
const RH_KEY_KINDS = {
    wallet: {rows: () => rhWalletKeyRows, setRows: v => { rhWalletKeyRows = v; }, listId: 'rhWalletKeysList', field: 'wallet_keys', firstField: 'wallet_api_key'},
    free: {rows: () => rhFreeKeyRows, setRows: v => { rhFreeKeyRows = v; }, listId: 'rhFreeKeysList', field: 'free_keys', firstField: 'api_key'}
};
function renderRhKeysList(kind){
    const conf = RH_KEY_KINDS[kind];
    if(!conf) return;
    const box = document.getElementById(conf.listId);
    if(!box) return;
    const rows = conf.rows();
    if(!rows.length) rows.push({key:'', note:''});
    const placeholderText = kind === 'wallet' ? (tr('api.rhWalletPlaceholder') || '请输入账户余额 API Key') : (tr('api.rhEnterCoinKey') || '请输入 RH币 API Key');
    box.innerHTML = rows.map((row, i) => {
        const statusTitle = `${escapeAttr(row.error || '')}${row.status_at ? ` · ${new Date(row.status_at).toLocaleString()}` : ''}`;
        return `
        <div class="rh-wallet-key-row ${row.enabled === false ? 'row-disabled' : ''}" data-idx="${i}">
            <button class="key-btn" type="button" onclick="moveRhApiKeyRow('${kind}', ${i}, -1)" title="上移（排序）" ${i === 0 ? 'disabled style="opacity:.35"' : ''}><i data-lucide="chevron-up" class="w-4 h-4"></i></button>
            <button class="key-btn" type="button" onclick="moveRhApiKeyRow('${kind}', ${i}, 1)" title="下移（排序）" ${i === rows.length - 1 ? 'disabled style="opacity:.35"' : ''}><i data-lucide="chevron-down" class="w-4 h-4"></i></button>
            <div class="field-frame" style="flex:1 1 auto"><input type="text" placeholder="${escapeAttr(row.placeholder || placeholderText)}" value="${escapeAttr(row.key || '')}" oninput="rhApiKeyRowChange('${kind}', ${i}, 'key', this.value)"></div>
            <div class="field-frame" style="flex:0 0 130px"><input type="text" placeholder="备注（可选）" value="${escapeAttr(row.note || '')}" oninput="rhApiKeyRowChange('${kind}', ${i}, 'note', this.value)"></div>
            ${row.enabled !== false && row.status ? `<span class="rh-key-status ${row.status}" title="${statusTitle}">${row.status === 'insufficient' ? '余额不足' : '报错'}</span>` : ''}
            ${row.enabled === false ? '<span class="rh-key-status off">已禁用</span>' : ''}
            ${row.is_next ? '<span class="rh-key-flag" title="游标轮换：这一把是下一次提交优先使用的">下一个</span>' : ''}
            <span class="rh-key-count${Number(row.today_success) > 0 ? '' : ' zero'}" title="这把钥匙今天的提交成功次数">${Number(row.today_success) > 0 ? `今日成功 ${row.today_success} 次` : '今日 0 次'}</span>
            <button class="key-btn ${row.enabled === false ? 'key-disabled-on' : ''}" type="button" onclick="toggleRhKeyEnabled('${kind}', ${i})" title="${row.enabled === false ? '已手动禁用（轮换自动跳过）：点击启用' : '点击禁用这把密钥（轮换时自动跳过）'}"><i data-lucide="${row.enabled === false ? 'power-off' : 'power'}" class="w-4 h-4"></i></button>
            <button class="key-btn key-clear" type="button" onclick="removeRhApiKeyRow('${kind}', ${i})" title="删除此密钥"><i data-lucide="x" class="w-4 h-4"></i></button>
        </div>`;
    }).join('');
    try{ lucide.createIcons(); }catch(e){}
}
function addRhApiKeyRow(kind){
    const conf = RH_KEY_KINDS[kind];
    if(!conf) return;
    const rows = conf.rows();
    rows.push({key:'', note:''});
    conf.setRows(rows);
    renderRhKeysList(kind);
}
function removeRhApiKeyRow(kind, i){
    const conf = RH_KEY_KINDS[kind];
    if(!conf) return;
    const rows = conf.rows();
    rows.splice(i, 1);
...
function moveRhApiKeyRow(kind, i, dir){
    const conf = RH_KEY_KINDS[kind];
    if(!conf) return;
    const rows = conf.rows();
    const j = i + dir;
...
function rhApiKeyRowChange(kind, i, field, value){
    const conf = RH_KEY_KINDS[kind];
    if(!conf) return;
    const row = conf.rows()[i];
    if(row) row[field] = value;
}
function collectRhApiKeys(kind){
    const conf = RH_KEY_KINDS[kind];
    if(!conf) return [];
    return conf.rows()
        .map(row => ({key: (row.key || '').trim(), note: (row.note || '').trim()}))
        .filter(it => it.key);
}
async function postRhKeyState(url, body){
    const res = await fetch(url, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body)});
    const data = await res.json().catch(() => ({}));
    if(!res.ok || data.success === false) throw new Error(typeof data.detail === 'string' ? data.detail : '操作失败');
    return data;
}
async function toggleRhKeyEnabled(kind, i){
    const conf = RH_KEY_KINDS[kind];
    if(!conf) return;
    const row = conf.rows()[i];
    if(!row?.key) return;
    try{
        const data = await postRhKeyState('/api/runninghub/key-state', {kind, key:row.key, enabled:row.enabled === false});
        conf.setRows((data.keys || []).map(it => ({key:it.key||'', note:it.note||'', today_success:Number(it.today_success)||0, is_next:it.is_next===true, enabled:it.enabled!==false, status:String(it.status||''), error:String(it.error||''), status_at:Number(it.status_at)||0})));
        renderRhKeysList(kind);
        setStatus(row.enabled === false ? '密钥已启用' : '密钥已禁用，轮换时将自动跳过');
    }catch(e){
        setStatus(`操作失败：${e.message}`);
    }
}
async function clearRhKeyStatus(kind){
    const label = kind === 'wallet' ? '余额密钥' : 'RH币密钥';
    if(!confirm(`清除全部${label}的失败状态（余额不足/报错），并把今日成功计数归零？\n游标与手动禁用开关保留。`)) return;
    const conf = RH_KEY_KINDS[kind];
    try{
        const data = await postRhKeyState('/api/runninghub/key-state/clear', {kind});
        conf.setRows((data.keys || []).map(it => ({key:it.key||'', note:it.note||'', today_success:Number(it.today_success)||0, is_next:it.is_next===true, enabled:it.enabled!==false, status:String(it.status||''), error:String(it.error||''), status_at:Number(it.status_at)||0})));
        renderRhKeysList(kind);
        setStatus('已清除全部失败状态，今日成功计数已归零');
    }catch(e){
        setStatus(`操作失败：${e.message}`);
    }
}
async function saveRhKeyOnly(kind){
    const item = provider();
    if(!item || item.id !== 'runninghub') return;
    const keys = collectRhApiKeys(kind);
    const label = kind === 'wallet' ? '钱包 Key' : 'RH币 Key';
    if(!keys.length){ alert(`请至少填写一把${label}`); return; }
    item[RH_KEY_KINDS[kind].field] = keys;
    item[RH_KEY_KINDS[kind].firstField] = keys[0].key;
    syncEditor();
    const ok = await saveProviders();
    if(ok){
        if(kind === 'wallet'){ rhWalletKeyRows = keys.map(it => ({key: it.key || '', note: it.note || ''})); }
        else { rhFreeKeyRows = keys.map(it => ({key: it.key || '', note: it.note || ''})); }
        renderRhKeysList(kind);
    }
}
async function clearRhKeyOnly(kind){
    const item = provider();
    if(!item || item.id !== 'runninghub') return;
    if(!confirm(tr('api.confirmClearKey') || '确认清除全部 Key？')) return;
    if(kind === 'wallet'){
        item._clearWalletKey = true;
        item.wallet_keys = [];
        rhWalletKeyRows = [];
    } else {
        item._clearKey = true;
...
async function saveProviders(){
    ...
    try {
        const res = await fetch('/api/providers', {
            ...
            wallet_keys:item.id === 'runninghub' ? (item.wallet_keys || []) : [],
            free_keys:item.id === 'runninghub' ? (item.free_keys || []) : [],
