...
const smartComposerQueueBtn = document.getElementById('smartComposerQueueBtn');
...
const smartQueueToggle = document.getElementById('smartQueueToggle');
const smartQueueClose = document.getElementById('smartQueueClose');
...
// ===== LLM 系统提示词技能预设：下拉选择后自动填充系统提示词 =====
const LLM_SKILL_PRESETS = [
    { id: 'skill-prompt-opt', name: '提示词优化师', prompt: '你是资深 AI 生图提示词工程师。把用户的简单想法改写为高质量生图提示词，依次覆盖：主体与外观、动作与姿态、场景与环境、光影与氛围、镜头与构图、风格与画质；保留用户原意，不添加无关元素；只输出提示词本身，不要解释。' },
    { id: 'skill-drama', name: '文戏分镜师(10-15秒)', prompt: '你是短剧文戏导演。把用户给出的剧情或对白改写为 10-15 秒可拍摄单元：【人物站位】场景、轴线、角色左右关系与道具状态；【分镜】2-4 镜，每镜标注时间段与景别，一个主导动作加 1-3 个可见表演信号；同一时刻只有一人说话；台词不超过 40 字；结尾定格在下一幕起点。只输出分镜脚本。' },
    { id: 'skill-h3', name: 'H3 视频提示词', prompt: '你是视频生成提示词工程师(MiniMax H3)。将用户描述改写为结构化视频提示词：画面主体与运动、镜头运动与时长、环境音与人声(不含背景音乐)、关键帧描述；中文输出，直接给结果。输出末尾另起一行标注素材引用：refs: 角色名1, 角色名2, 场景名（提示词中提到的角色和场景，没有则写 refs: 无）。' },
    { id: 'skill-translate', name: '中英互译', prompt: '你是专业翻译。用户输入中文则译成地道英文，输入英文则译成流畅中文；保留专有名词；只输出译文。' },
];
// 技能下拉行的高度（仅 LLM 启用但未开系统提示词时额外计入节点高度）
const PROMPT_LLM_SKILL_ROW_H = 30;
// 技能包（目录含 SKILL.md）由后端 /api/skills 扫描加载，启动后动态并入技能下拉
let llmSkillPackages = [];
function allLlmSkills(){
    return LLM_SKILL_PRESETS.concat(llmSkillPackages.filter(p => p && p.id && !LLM_SKILL_PRESETS.some(s => s.id === p.id)));
}
function llmSkillOptions(node){
    const selected = node.llmSystemSkillId || '';
    const customSelected = allLlmSkills().some(s => s.id === selected) ? '' : ' selected';
    return '<option value=""' + customSelected + '>自定义(手输系统提示词)</option>' +
        allLlmSkills().map(s => `<option value="${s.id}"${s.id === selected ? ' selected' : ''}>${escapeHtml(s.name)}</option>`).join('');
}
let promptLibraries = [];
...
const activeRhWorkflowPolls = new Map();
...
function performUndo(){
    ...
    undoSuppressed = true;
    nodes = snap.nodes;
    // 撤销删除：把恢复回来的节点从墓碑移除并同步服务端，避免它们被当"已删"过滤掉
    const restoredIds = (snap.nodes || []).map(n => String(n.id));
    restoredIds.forEach(id => deletedSmartNodeIds.delete(id));
    reportSmartTombstones([], restoredIds);
...
// 上传节点标题栏高度（与 CSS .node-head 的 height 一致），媒体区域需扣除该高度。
const SMART_NODE_HEAD_H = 38;
...
function promptNodeExpandedHeight(node){
    ...
    // 技能下拉只要启用 LLM 就常显（不依赖「系统提示词」开关）；未开开关时也要为其留出高度。
    const skillRowExtra = node?.llmEnabled && !node?.llmSystemEnabled ? PROMPT_LLM_SKILL_ROW_H : 0;
    return (node?.llmSystemEnabled ? 420 : 360) + skillRowExtra + smartNodeInputThumbsHeight(promptNodeInputImages(node)) + extra + upstreamExtra + promptNodeSplitExtraHeight(node);
}
...
function renderAngleControl(){
    ...
    const isAuto = ca.h === 0 && ca.v === 0 && ca.d === 4;
    return `<div class="smart-control angle-control">
        <button class="smart-pill" type="button" onclick="openAnglePanel()" title="${escapeAttr('打开3D角度控制面板')}"><i data-lucide="rotate-3d"></i><span>${isAuto ? escapeHtml(tr('smart.angleAuto') || '角度') : escapeHtml(tr('smart.angleCustom') || '角度·自定义')}</span><i data-lucide="chevron-right" class="pill-caret"></i></button>
    </div>`;
}
...
function renderedInputMediaRefs(){
    ...
    return [...inputThumbsRow.querySelectorAll('.input-thumb')].filter(el => el.dataset.refOff !== '1').map((el, index) => ({
        url:el.dataset.url || '',
...
async function loadConfig(){
    try {
        ...
        // 加载技能包（目录含 SKILL.md，由后端 /api/skills 扫描）；已选中技能包的节点重渲染以恢复选中态
        const skillsRes = await fetch('/api/skills').then(r => r.ok ? r.json() : null).catch(() => null);
        if(skillsRes && Array.isArray(skillsRes.skills)){
            llmSkillPackages = skillsRes.skills;
            if(selectedNode()?.type === 'smart-prompt' && selectedNode().llmSystemSkillId && allLlmSkills().some(s => s.id === selectedNode().llmSystemSkillId)){
                render();
            }
        }
...
function smartNodeInFlight(node){
    ...
    return Boolean(node && (node.running || node.pending || node.queued || node.jimengPending || node.rhWorkflowTaskId || smartPendingTasks(node).length));
}
...
function smartNodeHasCompletedResult(node){
    if(node?.rhWorkflowTaskId) return false;
...
// ===== displayTitle 独立标题字段：用户自定义标题不受生成结果覆盖 =====
const SMART_DEFAULT_TITLES = new Set([
    '','Image','IMAGE','image','Group','GROUP','group',
    'Video','Videos','Audio','Audios','Text','Texts','File',
    'Prompt','Loop','MiniMax','MiniMax H3','Upscale','Upload',
    'Grid','Grid Join','智能分组','分组','万能分组','历史分组',
    'Videos','Audios','Texts'
]);
function isSmartDefaultTitle(title){
    return SMART_DEFAULT_TITLES.has(String(title || '').trim());
}
/** 获取节点显示标题：displayTitle → 可迁移的自定义 title → null（调用方补充默认值） */
function smartNodeDisplayTitle(node){
    if(node?.displayTitle) return node.displayTitle;
    if(node?.title && !isSmartDefaultTitle(node.title)) return node.title;
    return null;
}
/** 迁移旧节点：将自定义 title 迁移到 displayTitle（仅执行一次） */
function migrateNodeDisplayTitle(node){
    if(!node || node.displayTitle) return;
    if(node.title && !isSmartDefaultTitle(node.title)){
...
function syncRunButtonState(node=selectedNode()){
    ...
    syncSmartComposerQueueBtn(node);
}
function syncSmartComposerQueueBtn(node=selectedNode()){
    if(!smartComposerQueueBtn) return;
    const inQueue = Boolean(node?.id && smartRunQueue.includes(node.id));
    smartComposerQueueBtn.disabled = !isSmartRunnableNode(node) || smartCascadeIsLoopRunning(node?.id);
    smartComposerQueueBtn.classList.toggle('in-queue', inQueue);
    const span = smartComposerQueueBtn.querySelector('span');
    if(span) span.textContent = inQueue ? '已入列' : '进入列队';
    const icon = smartComposerQueueBtn.querySelector('svg[data-lucide], i[data-lucide]');
    if(icon){
        const name = inQueue ? 'check' : 'list-plus';
        if(icon.dataset.lucide !== name){
            icon.setAttribute('data-lucide', name);
            icon.setAttribute('class', `lucide lucide-${name}`);
            if(window.lucide?.icons?.[name]) icon.innerHTML = window.lucide.icons[name];
        }
    }
}
function mergeSmartNode(local, remote){
    ...
    if(remoteDone && localBusy && !localDone){
        // 远端先完成时只接收生成结果和完成状态，保留本地正在使用的自定义标题与布局，避免标题恢复为 Image。
        return completeSmartNodeWithImages({...remote, ...local}, images);
    }
    if(localDone && remoteDone){
        // 双方均完成：布局/标题/设置仍以本地为准（撤销拖动/编辑不被服务端旧快照盖回），完成时间取较新者，图片取并集
        const finishedAt = Math.max(Number(local.runFinishedAt || 0), Number(remote.runFinishedAt || 0));
        return completeSmartNodeWithImages({...local, images, runFinishedAt: finishedAt}, images);
    }
    // 本地正在生成/排队的节点完全以本地为准，只把对方可能多出来的图并进来，绝不被对方旧状态冲掉
    if(smartNodeInFlight(local)){
        return {...local, images};
    }
    // 非运行中的节点：以本地布局/标题/设置为基底（拖动后的位置不被服务端旧快照盖回），只并入双方图片并集
...
// 本地已删除节点的墓碑集合：弱网/合并时，服务端旧快照里仍残留这些 id，遇到就跳过、不复活
const deletedSmartNodeIds = new Set();
// 墓碑增量上报：删除/撤销删除瞬间同步到服务端（不依赖主 save 的 450ms 防抖与网络状况），
// 弱网下即便主保存失败，刷新加载时服务端 get 也会按墓碑过滤，节点不会再复活。
function reportSmartTombstones(addIds = [], removeIds = []){
    if(!canvasId) return;
    try {
        fetch(`/api/canvases/${encodeURIComponent(canvasId)}/tombstones`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({add: (addIds || []).map(String), remove: (removeIds || []).map(String)})
        }).catch(() => {});
    } catch(e) {}
}
...
function mergeSmartNodeLists(localNodes, remoteNodes){
    ...
    return order.map(id => {
        ...
        if(remote && !local){
            if(deletedSmartNodeIds.has(id)) return null;  // 本会话删过的节点：服务端旧快照不再救活
            return remote;                        // 仅对方存在：加入对方新建的节点
        }
...
function applyMergedServerCanvas(serverCanvas){
    ...
    // 应用服务端墓碑：其他端删掉的节点，本端也一并剔除，避免本地旧缓存把已删节点写回救活
    const serverTombstones = (serverCanvas.deleted_node_ids || []).map(String);
    serverTombstones.forEach(id => deletedSmartNodeIds.add(id));
    if(serverTombstones.length){
        nodes = nodes.filter(n => !deletedSmartNodeIds.has(String(n.id)));
    }
...
async function loadCanvas(){
    ...
    try {
        ...
        // 重建删除墓碑：以服务端持久化的删除记录为准，弱网刷新后仍能挡住已删节点的复活
        deletedSmartNodeIds.clear();
        (canvas.deleted_node_ids || []).forEach(id => deletedSmartNodeIds.add(String(id)));
        if(deletedSmartNodeIds.size){
            nodes = nodes.filter(n => !deletedSmartNodeIds.has(String(n.id)));
        }
        migrateSmartGroupImageMembers();
        // 迁移旧节点：将自定义 title 迁移到 displayTitle
        nodes.forEach(migrateNodeDisplayTitle);
        canvas.connections = Array.isArray(canvas.connections) ? canvas.connections : [];
        nodes.forEach(n => {
            if(n.type === 'smart-minimax') n.timelinePlaying = false;
            const pendingTasks = smartPendingTasks(n);
            if(n.rhWorkflowTaskId){
                // RunningHub 工作流任务的持久标记：刷新后交由 resumeSmartPendingTasks → resumeSmartPendingNode
                // 恢复轮询。这里不清 pending / 不置完成，否则会被误判成陈旧残留，导致计时消失、轮询不恢复。
                return;
            }
...
function createLoopNode(x, y, options={}){
    if(!options.skipUndo) pushUndo();
    const node = {id:uid('loop'), type:'smart-loop', x, y, w:340, h:168, title:'Loop', displayTitle:'Loop', count:1, mode:'serial', showPrompt:false, imageInput:false, loopStart:1, imageBatchSize:1, variablePrompt:'', created_at:Date.now()};
    nodes.push(node);
    if(options.select !== false) selectedId = node.id;
    render();
    scheduleSave();
...
function createSmartGroupNode(x, y, options={}){
    if(!options.skipUndo) pushUndo();
    const node = {id:uid('group'), type:'smart-group', x, y, w:SMART_GROUP_DEFAULT_WIDTH, h:SMART_GROUP_DEFAULT_HEIGHT, title:'智能分组', displayTitle:'智能分组', items:[], created_at:Date.now()};
    nodes.push(node);
    if(options.select !== false) selectedId = node.id;
    render();
    scheduleSave();
...
function nextSmartNodeTitle(node, reservedTitles=[]){
    const title = String(node?.displayTitle || node?.title || '').trim();
    if(!title) return '';
    const match = title.match(/^(.*?)(?:\s+(\d+))?$/);
    const base = String(match?.[1] || title).trim();
    const start = Number(match?.[2] || 1) + 1;
    const occupied = new Set(
        nodes
            .filter(item => item.type === node.type && item.id !== node.id)
            .map(item => String(item.displayTitle || item.title || '').trim())
            .concat(reservedTitles)
    );
    let index = start;
...
function cloneSmartNode(node, dx=0, dy=0, reservedTitles=[]){
    const copy = JSON.parse(JSON.stringify(node));
    copy.displayTitle = nextSmartNodeTitle(node, reservedTitles);
...
function pasteNodes(){
    ...
    const copies = sourceNodes.map(n => {
        const copy = cloneSmartNode(n, dx, dy, reservedTitles);
        if(copy.title) reservedTitles.push(copy.title);
...
function moveNodeElementsDuringDrag(){
    ...
    groupItems.map(item => item.id).forEach(id => {
        ...
        if(n && el){
            // 拖拽期间用 transform:translate 实现 GPU 合成，避免 left/top 触发布局重排导致掉帧
...
function promptNodeBodyHtml(node){
    ...
    const llmParams = node.llmEnabled ? `
        <div class="prompt-node-llm">
            ...
            ${node.llmEnabled ? `<div class="prompt-llm-skill-row"><span class="prompt-llm-skill-label">技能</span><select class="prompt-node-control prompt-llm-skill">${llmSkillOptions(node)}</select></div>` : ''}
                ...
                <div class="minimax-current-panel">
                    ...
                    <div class="minimax-clip-parameters">
                        ...
                        <div class="minimax-settings minimax-segment-fields">
                            ...
                            <button class="prompt-node-queue minimax-queue ${smartRunQueue.includes(node.id) ? 'in-queue' : ''}" type="button" data-minimax-queue="${escapeAttr(node.id)}" title="加入运行列队，点开始后逐个自动运行"><i data-lucide="list-plus"></i><span>${smartRunQueue.includes(node.id) ? '已入列' : '进入列队'}</span></button>
                        </div>
                    </div>
                </div>
            </div>
...
function nodeBodyHtml(node, layout){
    ...
    if(imgs[0]) {
        const mediaH = isImageNode ? Math.max(40, layout.height - SMART_NODE_HEAD_H) : layout.height;
        return `<div class="image-wrap has-outside-image-name ${selectedImage.nodeId === node.id && selectedImage.index === 0 ? 'image-selected' : ''}" data-image-index="0" data-media-signature="${escapeAttr(`${mediaKindForItem(imgs[0])}:${imgs[0]?.url || ''}`)}" style="--node-img-w:${layout.width}px;--node-img-h:${mediaH}px">${singleMediaHtml(imgs[0], layout.width, mediaH)}${imageNameBadgeHtml(imgs[0], {outside:true})}${imageResolutionBadgeHtml(imgs[0])}<button class="mini-x image-delete" type="button" data-image-index="0" title="${escapeHtml(tr('smart.deleteImage'))}"><i data-lucide="trash-2"></i></button></div>`;
    }
...
function runTimePillHtml(node){
    ...
    const running = Boolean(node.pending || node.running || node.jimengPending || node.rhWorkflowTaskId);
...
function hideRunTimerForNode(node){
    if(!node || node.runTimerHidden || node.pending || node.running || node.jimengPending || node.rhWorkflowTaskId || !node.runFinishedAt) return false;
    node.runTimerHidden = true;
    scheduleSave();
    return true;
}
function refreshRunTimerPills(){
    const active = nodes.some(n => n.type !== 'smart-prompt' && !n.runTimerHidden && (n.pending || n.running || n.jimengPending || n.rhWorkflowTaskId || n.runFinishedAt));
    document.querySelectorAll('[data-run-timer]').forEach(el => {
        ...
        el.classList.toggle('done', Boolean(!node.pending && !node.running && !node.jimengPending && !node.rhWorkflowTaskId && node.runFinishedAt));
    });
...
function render(){
    ...
    const nodeHtmlEntries = nodes
        ...
        const ndDispTitle = smartNodeDisplayTitle(node);
        const title = node.type === 'smart-group' ? (node.title === '万能分组' ? '智能分组' : (node.title || '智能分组')) : node.type === 'smart-prompt' ? 'Prompt' : node.type === 'smart-loop' ? 'Loop' : node.type === 'smart-minimax' ? 'MiniMax H3' : escapeHtml(ndDispTitle || node.title || (imgs.length > 1 ? 'Group' : imgs.length ? 'Image' : tr('smart.createImportNode')));
        ...
        const isImageNode = node.type === 'smart-image' || !node.type;
        const isEditableTitle = isImageNode && imgs.length <= 1;
        ...
        const html = `<div class="image-node ${isEmpty ? 'empty-node' : ''} ${isGroup ? 'group-node' : ''} ${isHistory ? 'history-group-node' : ''} ${isPrompt ? 'prompt-smart-node' : ''} ${isLoop ? 'loop-smart-node' : ''} ${isMinimax ? 'minimax-smart-node' : ''} ${isSmartGroup ? 'smart-group-node' : ''} ${isCompactMember ? 'smart-group-member-node' : ''} ${isEditableTitle ? 'upload-title-node' : ''} ${isNodeSelected(node.id) ? 'selected' : ''} ${(dragState?.groupIds?.includes(node.id) || dragState?.id === node.id) ? 'dragging' : ''} ${node.running ? 'node-running' : ''} ${isPending ? 'node-pending' : ''}" data-id="${escapeHtml(node.id)}" style="left:${node.x || 0}px;top:${node.y || 0}px;width:${layout.width}px;height:${layout.height}px">

            <div class="node-head"><div class="node-title" ${isEditableTitle ? 'contenteditable="true" data-node-title="1"' : ''}>${title}</div><div class="node-actions">${deleteBtn}</div></div>
...
function bindPromptNodeControls(el, node){
    ...
    const skillSelEl = el.querySelector('.prompt-llm-skill');
    if(skillSelEl){ skillSelEl.onmousedown = e => e.stopPropagation(); skillSelEl.onchange = e => {
        e.stopPropagation();
        const id = e.target.value;
        node.llmSystemSkillId = id || '';
        const skill = allLlmSkills().find(s => s.id === id);
        if(skill){
            node.llmSystemPrompt = skill.prompt;
            const ta = el.querySelector('.prompt-llm-system');
            if(ta) ta.value = skill.prompt;
            scheduleSave();
        }
    }; }
...
// ===== 运行列队：进入列队 → 右侧列队栏 → 逐个自动运行 =====
let smartRunQueue = [];
let smartQueueRunning = false;
let smartQueueRuns = {}; // nodeId -> {status:'waiting'|'running'|'done'|'error', label}
let smartQueuePanelEl = null;
let smartQueueToggleEl = null;
function smartQueueNodeName(node){
    if(!node) return '未知节点';
    const dn = node.displayTitle || node.title;
    if(node.type === 'smart-prompt') return dn || 'Prompt';
    if(node.type === 'smart-minimax') return dn || 'MiniMax';
    if(node.type === 'smart-loop') return dn || 'Loop';
    if(node.type === 'smart-group') return dn || '分组';
    return dn || ((node.images || []).length ? 'Image' : '上传');
}
function queueToggleNode(nodeId){
    if(smartRunQueue.includes(nodeId)) queueRemoveNode(nodeId);
    else queueAddNode(nodeId);
}
function queueAddNode(nodeId){
    if(smartRunQueue.includes(nodeId)) return;
    smartRunQueue.push(nodeId);
    smartQueueRuns[nodeId] = {status:'waiting', label:'等待中'};
    renderSmartQueuePanel();
    render();
    syncSmartComposerQueueBtn(selectedNode());
}
function queueRemoveNode(nodeId){
    smartRunQueue = smartRunQueue.filter(id => id !== nodeId);
    delete smartQueueRuns[nodeId];
    renderSmartQueuePanel();
    render();
    syncSmartComposerQueueBtn(selectedNode());
}
function queueMoveNode(nodeId, dir){
    const i = smartRunQueue.indexOf(nodeId);
    const j = i + dir;
    if(i < 0 || j < 0 || j >= smartRunQueue.length) return;
    [smartRunQueue[i], smartRunQueue[j]] = [smartRunQueue[j], smartRunQueue[i]];
    renderSmartQueuePanel();
}
async function smartQueueRunNode(node){
    const beforeTotal = nodes.reduce((sum, n) => sum + (n.images || []).filter(img => img?.url).length, 0);
    try {
        if(node.type === 'smart-minimax'){
            await runMinimaxNode(node.id);
            if(smartMinimaxSelectedSegment(node)?.result?.url) return {status:'done', label:'完成'};
            if(node.running || node.pending) return {status:'running', label:'生成中'};
            return {status:'error', label:'失败'};
        }
        selectedId = node.id;
        selectedIds = [];
        await runGeneration();
        const afterTotal = nodes.reduce((sum, n) => sum + (n.images || []).filter(img => img?.url).length, 0);
        if(afterTotal > beforeTotal) return {status:'done', label:'完成'};
        if(node.pending || node.running || smartPendingTasks(node).length || node.jimengPending) return {status:'running', label:'生成中'};
        return {status:'error', label:'失败'};
    } catch(e) {
        return {status:'error', label:'失败'};
    }
...
async function queueStart(){
    if(smartQueueRunning) return;
    const ids = smartRunQueue.slice();
    if(!ids.length){ toast('列队为空'); return; }
    ids.forEach(id => { smartQueueRuns[id] = {status:'waiting', label:'等待中'}; });
    smartQueueRunning = true;
    renderSmartQueuePanel();
    for(let i = 0; i < ids.length; i++){
        const id = ids[i];
        if(!smartRunQueue.includes(id)) continue;
        const node = nodes.find(n => n.id === id);
        if(!node){
            smartQueueRuns[id] = {status:'error', label:'节点已删除'};
            renderSmartQueuePanel();
            continue;
        }
        if(node.running) continue;
        smartQueueRuns[id] = {status:'running', label:'运行中…'};
        renderSmartQueuePanel();
        render();
        const result = await smartQueueRunNode(node);
        smartQueueRuns[id] = {status:result.status, label:result.label};
        renderSmartQueuePanel();
        render();
    }
    smartQueueRunning = false;
    renderSmartQueuePanel();
    render();
    syncSmartComposerQueueBtn(selectedNode());
    toast('列队运行完成');
...
function queueClearFinished(){
    smartRunQueue = smartRunQueue.filter(id => {
        const run = smartQueueRuns[id];
        return !run || (run.status !== 'done' && run.status !== 'error');
    });
    renderSmartQueuePanel();
    render();
    syncSmartComposerQueueBtn(selectedNode());
}
function smartQueueStatusBadge(run){
    const map = {
        waiting:['waiting', '等待中'],
        running:['running', '运行中…'],
        done:['done', '完成'],
    ...
    const entry = map[run?.status] || map.waiting;
    return `<span class="smart-queue-status ${entry[0]}">${escapeHtml(run?.label || entry[1])}</span>`;
}
function renderSmartQueuePanel(){
    smartQueuePanelEl = smartQueuePanelEl || document.getElementById('smartQueuePanel');
    if(!smartQueuePanelEl) return;
    const listEl = smartQueuePanelEl.querySelector('.smart-queue-list');
    if(listEl){
        if(!smartRunQueue.length){
            listEl.innerHTML = '<div class="smart-queue-empty">列队为空<br>选中节点后点输入区「进入列队」加入</div>';
        } else {
            listEl.innerHTML = smartRunQueue.map((id, idx) => {
                const node = nodes.find(n => n.id === id);
                const name = escapeHtml(smartQueueNodeName(node));
                const run = smartQueueRuns[id] || null;
                const runCls = run?.status === 'running' ? 'running' : '';
                const statusHtml = smartQueueStatusBadge(run);
                return `<div class="smart-queue-item ${runCls} ${run?.status === 'done' ? 'done' : ''} ${run?.status === 'error' ? 'error' : ''}" data-nid="${escapeHtml(id)}">
                    <span class="smart-queue-index">${idx + 1}</span>
                    <span class="smart-queue-name" title="${name}">${name}</span>
                    ${statusHtml}
                    <span class="smart-queue-actions">
                        <button class="smart-queue-btn" type="button" title="上移" data-qmove="-1"><i data-lucide="chevron-up"></i></button>
                        <button class="smart-queue-btn" type="button" title="下移" data-qmove="1"><i data-lucide="chevron-down"></i></button>
                        <button class="smart-queue-btn" type="button" title="移除" data-qremove="1"><i data-lucide="x"></i></button>
                    </span>
                </div>`;
            }).join('');
            listEl.querySelectorAll('[data-qmove]').forEach(btn => btn.onclick = () => queueMoveNode(btn.closest('.smart-queue-item')?.dataset?.nid || '', Number(btn.dataset.qmove)));
            listEl.querySelectorAll('[data-qremove]').forEach(btn => btn.onclick = () => queueRemoveNode(btn.closest('.smart-queue-item')?.dataset?.nid || ''));
        }
    }
    const startBtn = smartQueuePanelEl.querySelector('.smart-queue-start');
    if(startBtn){
        startBtn.disabled = smartQueueRunning || !smartRunQueue.length;
        const span = startBtn.querySelector('span');
        if(span) span.textContent = smartQueueRunning ? '运行中…' : '开始';
    }
    if(window.lucide) lucide.createIcons();
}
function toggleSmartQueuePanel(open){
    smartQueuePanelEl = smartQueuePanelEl || document.getElementById('smartQueuePanel');
    smartQueueToggleEl = smartQueueToggleEl || document.getElementById('smartQueueToggle');
    if(!smartQueuePanelEl) return;
    const willOpen = typeof open === 'boolean' ? open : !smartQueuePanelEl.classList.contains('open');
    smartQueuePanelEl.classList.toggle('open', willOpen);
    smartQueueToggleEl?.classList.toggle('active', willOpen);
    if(willOpen) renderSmartQueuePanel();
}

// ===== 节点右键菜单 + 类型转换（提示词 ↔ 上传） =====
let smartNodeContextMenu = null;
function openNodeContextMenu(node, e, convertible=true){
    closeNodeContextMenu();
    const menu = document.createElement('div');
    menu.className = 'smart-node-context-menu';
    const isPrompt = node.type === 'smart-prompt';
    const targetFirst = isPrompt ? 'smart-image' : 'smart-prompt';
    const targetSecond = isPrompt ? 'smart-prompt' : 'smart-image';
    const convertHtml = convertible ? `
        <button class="smart-node-context-item" type="button" data-convert="1"><i data-lucide="repeat"></i><span>节点转换</span></button>
        <div class="smart-node-convert-options" hidden>
            <button class="smart-node-context-item convert" type="button" data-target="${targetFirst}">转换为${isPrompt ? '上传' : '提示词'}</button>
            <button class="smart-node-context-item convert" type="button" data-target="${targetSecond}">转换为${isPrompt ? '提示词' : '上传'}</button>
        </div>` : '';
    const selectionIds = selectedIds.includes(node.id) ? selectedIds.slice() : [node.id];
    const groupableIds = selectionIds.filter(id => {
        const selectedNode = nodes.find(item => item.id === id);
        return selectedNode && !isSmartGroupNode(selectedNode);
    });
    const runnableIds = selectionIds.filter(id => {
        const selectedNode = nodes.find(item => item.id === id);
        return isSmartRunnableNode(selectedNode) && !smartNodeInFlight(selectedNode);
    });
    ...
    const groupBtn = menu.querySelector('[data-selection-group]');
    if(groupBtn) groupBtn.onclick = e => {
        e.stopPropagation();
        closeNodeContextMenu();
        groupSelectedNodes();
    };
    const runSelectionBtn = menu.querySelector('[data-selection-run]');
    if(runSelectionBtn) runSelectionBtn.onclick = e => {
        e.stopPropagation();
        closeNodeContextMenu();
        runnableIds.forEach(queueAddNode);
    ...
    menu.querySelectorAll('[data-target]').forEach(btn => {
        btn.onclick = () => { convertSmartNode(node, btn.dataset.target); closeNodeContextMenu(); };
    });
    menu.addEventListener('mousedown', e => e.stopPropagation());
    menu.addEventListener('contextmenu', e => { e.preventDefault(); e.stopPropagation(); });
    if(window.lucide) lucide.createIcons();
    smartNodeContextMenu = menu;
}
...
function convertSmartNode(node, targetType){
    ...
    if(targetType === 'smart-prompt'){
        if(node.text === undefined) node.text = '';
        if(node.llmEnabled === undefined) node.llmEnabled = true;
        if(node.llmSystemEnabled === undefined) node.llmSystemEnabled = false;
        if(node.llmSystemPrompt === undefined) node.llmSystemPrompt = '';
        if(node.llmInstruction === undefined) node.llmInstruction = '';
        if(node.llmProvider === undefined) node.llmProvider = '';
        if(node.llmModel === undefined) node.llmModel = '';
        if(node.llmSystemSkillId === undefined) node.llmSystemSkillId = '';
    } else {
        if(node.images === undefined) node.images = [];
        delete node.queued;
        delete node.pending;
    }
    delete node.running;
    ...
    render();
    scheduleSave();
    toast(`节点已转换为${targetType === 'smart-prompt' ? '提示词' : '上传'}`);
}
...
function bindMinimaxNodeControls(el, node){
    ...
    el.querySelectorAll('[data-minimax-queue]').forEach(btn => {
        btn.onclick = e => {
            e.preventDefault();
            e.stopPropagation();
            queueToggleNode(btn.dataset.minimaxQueue || node.id);
        };
    });
...
function bindNodeEvents(){
    world.querySelectorAll('.image-node').forEach(el => {
        ...
        const convertible = nodeForControls && (nodeForControls.type === 'smart-prompt' || nodeForControls.type === 'smart-image' || !nodeForControls.type);
        el.oncontextmenu = e => {
            ...
            const node = nodes.find(n => n.id === id);
            if(!node) return;
            openNodeContextMenu(node, e, convertible);
        };
        el.onclick = e => {
            e.stopPropagation();
        ...
        el.querySelectorAll('.node-title[data-node-title]').forEach(titleEl => {
        ...
        const beginNodeDrag = e => {
            ...
            if(e.target.closest('.prompt-node-pill, .node-title[contenteditable="true"], textarea:not(.prompt-node-text)')) return;
            e.preventDefault(); e.stopPropagation();
...
function deleteNode(id){
    ...
    deleteIds.forEach(did => deletedSmartNodeIds.add(did));
    reportSmartTombstones([...deleteIds], []);
...
function deleteImage(id, imageIndex){
    ...
    if(node.images.length <= 1 && !node.displayTitle) node.title = 'Image';
...
function renderInputThumbsRow(node){
    ...
    const thumbsSignature = JSON.stringify({
        node: node?.id || '',
        items: dedup.map(img => `${inputRefKey(img)}@${img.url || ''}@${isInputRefVisuallyBlocked(node, img) ? 'off' : 'on'}`),
        manual: [...manualRefKeys],
        add: addActive,
        skip: node ? defaultRefSkipsSelfOutput(node) : false,
    ...
    const thumbsHtml = dedup.map((img, i) => {
        ...
        const refOff = isInputRefVisuallyBlocked(node, img);
        const eyeBtn = `<button class="input-thumb-eye" type="button" title="${escapeHtml(refOff ? '点击恢复为参考输入' : '点击排除（不作为参考输入）')}" aria-label="${escapeHtml(refOff ? '恢复参考输入' : '排除参考输入')}"><i data-lucide="${refOff ? 'eye-off' : 'eye'}"></i></button>`;
        return `<div class="input-thumb ${isSelf ? 'input-self' : ''} ${removable ? 'input-manual-ref' : ''} ${refOff ? 'input-blocked' : ''}" draggable="false" data-thumb-index="${i}" ${refOff ? 'data-ref-off="1" data-blocked-label="已排除"' : ''} data-node-id="${escapeHtml(img.nodeId || '')}" data-image-index="${img.imageIndex ?? ''}" data-url="${escapeHtml(img.url || '')}" data-source-url="${escapeHtml(sourceUrl)}" title="${escapeHtml(`${img.name || tr('smart.inputNum').replace('{n}', String(i + 1))} · ${title}`)}">${inner}<span class="input-thumb-label">${escapeHtml(label)}</span>${removeBtn}${eyeBtn}</div>`;
    }).join('');
    ...
    bindInputThumbReferenceActions();
    bindInputThumbEyeToggles(node, dedup);
    refreshIcons();
}
function bindInputThumbEyeToggles(node, items){
    if(!inputThumbsRow || !node) return;
    inputThumbsRow.querySelectorAll('.input-thumb-eye').forEach(btn => {
        btn.addEventListener('click', event => {
            event.preventDefault();
            event.stopPropagation();
            const thumb = btn.closest('.input-thumb');
            const item = items[Number(thumb?.dataset.thumbIndex ?? -1)];
            if(!item?.url) return;
            toggleInputRefVisibility(node, item);
            renderInputThumbsRow(node);
        });
    });
}
...
function appendImagesToSmartNode(uploaded, targetId='', opts={}){
    ...
    if(node.images.length > 1){
        ...
        if(!node.displayTitle) node.displayTitle = node.title;
    ...
    if(node.images.length === 1){ node.title = uploadTitleForItems(node.images, node.title || 'Image'); if(!node.displayTitle) node.displayTitle = node.title; delete node.w; delete node.h; }
...
// RH 工作流 / ComfyUI：节点自身上一轮的产出（generatedResult）默认不算参考输入，避免再次生成时旧产物占媒体槽位；
// 输入缩略图上的小眼睛可逐个覆盖：allowedSelfRefs 恢复被默认排除的自身产出，blockedInputRefs 手动排除任意参考。
function defaultRefSkipsSelfOutput(node){
    const source = node ? smartSettingsForNode(node) : settings;
    if(source.engine === 'comfy') return true;
    if(source.engine === 'runninghub' && rhCurrentKind(source) === 'workflow') return true;
    return false;
}
function selfRefAllowedKeys(node){
    return new Set(Array.isArray(node?.allowedSelfRefs) ? node.allowedSelfRefs.filter(Boolean) : []);
}
function selfRefAutoSkipped(node, img){
    return Boolean(defaultRefSkipsSelfOutput(node) && img?.generatedResult === true);
}
function isInputRefVisuallyBlocked(node, img){
    if(!node || !img?.url) return false;
    if(selfRefAutoSkipped(node, img)) return !selfRefAllowedKeys(node).has(inputRefKey(img));
    return isInputRefBlocked(node, img);
}
function toggleInputRefVisibility(node, img){
    if(!node || !img?.url) return;
    const key = inputRefKey(img);
    if(selfRefAutoSkipped(node, img)){
        const allowed = selfRefAllowedKeys(node);
        if(allowed.has(key)) allowed.delete(key); else allowed.add(key);
        node.allowedSelfRefs = [...allowed];
        if(!node.allowedSelfRefs.length) delete node.allowedSelfRefs;
        scheduleSave();
    } else {
...
function defaultReferenceImagesFor(node, consume=false, ctx=smartLoopContext){
    if(!node) return [];
    const selfAll = selfReferenceImagesForNode(node, consume, ctx).filter(img => img?.url);
    let self = selfAll;
    if(defaultRefSkipsSelfOutput(node)){
        const allowed = selfRefAllowedKeys(node);
        self = selfAll.filter(img => img?.generatedResult !== true || allowed.has(inputRefKey(img)));
    }
...
function visibleReferenceImagesFor(node){
    if(!node) return [];
    // 默认被排除的自身产出也要显示在输入缩略行（闭眼态），否则小眼睛没有恢复入口
    const hiddenSelf = defaultRefSkipsSelfOutput(node)
        ? selfReferenceImagesForNode(node).filter(img => img?.url && img.generatedResult === true)
        : [];
    const base = defaultReferenceImagesFor(node);
    return uniqueReferenceImages([...hiddenSelf, ...base, ...collectMentionedImagesFromPrompt()]);
}
...
function finalizePendingNode(pendingNode, urls, meta, kind='image'){
    ...
    // 有 displayTitle 时 title 仅为业务字段，不覆盖不影响显示
    if(pendingNode.displayTitle) delete pendingNode.title;
...
function restoreSourceVisualState(node, state){
    ...
    // 保留 displayTitle 不受恢复覆盖；title 仅作业务字段，回退到恢复前状态无影响
    if(!node.displayTitle) node.title = state.title || (node.images.length > 1 ? 'Group' : 'Image');
...
function finishLoopTargetPreviewState(node){
    ...
    if((node.images || []).some(img => img?.url)){
        ...
        if(node.displayTitle) delete node.title;
...
function showDirectLoopRoundPreview(loopNode, target, refs, loopIndex, total){
    ...
    if(!target.displayTitle) target.displayTitle = target.title;
    delete target.w;
    delete target.h;
    render();
    return true;
...
function appendOutputsToNode(node, additions, kind='image', options={}){
    ...
    markSmartNodeComplete(node);
    node.outputKind = kind;
    const generatedDefaultTitle = node.images.length > 1 ? (kind === 'video' ? 'Videos' : kind === 'audio' ? 'Audios' : kind === 'text' ? 'Texts' : 'Group') : (kind === 'video' ? 'Video' : kind === 'audio' ? 'Audio' : kind === 'text' ? 'Text' : kind === 'file' ? 'File' : 'Image');
    // 保留用户设置的节点名称，只有标题为空或仍是系统默认名时才更新。
    // 有 displayTitle 时 title 仅为业务字段，不更新不影响显示。
    const defaultTitles = new Set(['', 'Image', 'IMAGE', 'Group', 'Video', 'Videos', 'Audio', 'Audios', 'Text', 'Texts', 'File']);
    if(!node.displayTitle && defaultTitles.has(String(node.title || '').trim())) node.title = generatedDefaultTitle;
    delete node.w;
    delete node.h;
    const afterRight = (Number(node.x) || 0) + nodeRect(node).width;
    const skipShift = options.skipShift || Boolean(smartLoopContext?.nodeId);
...
function pollRunningHubWorkflowTask(taskId, useWallet=false){
    // 同一 taskId 只保留一个在途轮询：多标签页同步/刷新恢复时并发 resume 归到同一个 promise，
    // 避免两个轮询同时 finalize 造成节点状态互相覆盖（计时消失/重复清 pending）。
    if(activeRhWorkflowPolls.has(taskId)) return activeRhWorkflowPolls.get(taskId);
    const promise = runRunningHubWorkflowPoll(taskId, useWallet);
    activeRhWorkflowPolls.set(taskId, promise);
    promise.finally(() => {
        if(activeRhWorkflowPolls.get(taskId) === promise) activeRhWorkflowPolls.delete(taskId);
    }).catch(() => {});
    return promise;
}
async function runRunningHubWorkflowPoll(taskId, useWallet=false){
    let successEmptyCount = 0;
    for(let i = 0; i < 720; i++){
        await sleep(2500);
        ...
        if(data.status === 'SUCCESS'){
            ...
            // 已标成功但视频仍在写对象存储、url 尚未回填：继续等下一轮，别立即判"输出为空"失败
            successEmptyCount += 1;
            if(successEmptyCount >= 20) throw new Error(tr('smart.rhOutputsEmpty'));
            continue;
        }
...
async function runRunningHubGeneration(prompt, refs, runSettings=settings, pendingNode=null){
    ...
    // 持久化到节点，页面刷新后 resumeSmartPendingNode 能恢复轮询
    if(pendingNode){
        pendingNode.rhWorkflowTaskId = taskId;
        pendingNode.rhWorkflowUseWallet = useWallet;
        scheduleSave();
    }
    try {
        return await pollRunningHubWorkflowTask(taskId, useWallet);
    } finally {
        // 无论成功/失败/超时/异常，清除节点上的持久化 taskId，避免陈旧残留
        if(pendingNode){
            delete pendingNode.rhWorkflowTaskId;
            delete pendingNode.rhWorkflowUseWallet;
            scheduleSave();
        }
    }
}
...
async function runMinimaxRunningHub(node){
    ...
    const refs = smartMinimaxRunRefs(node);
    const urls = await runRunningHubGeneration(prompt, refs, runSettings, node);
...
async function resumeSmartPendingNode(node, logContext={}){
    if(!node) return;
    // 恢复 RunningHub 工作流轮询：页面刷新或中断后，节点上可能残留 rhWorkflowTaskId
    if(node.rhWorkflowTaskId){
        const taskId = node.rhWorkflowTaskId;
        const useWallet = node.rhWorkflowUseWallet === true;
        // 轮询期间保留 rhWorkflowTaskId 在节点上：它是「该节点正在跑此 RH 任务」的持久标记。
        // 若提前删除，轮询中途节点一旦被保存（多标签页同步/其它操作触发 save），其它端就会把
        // 它误判成「pending 残留」而自愈清零、停止计时。等轮询真正结束再清理。
        render();
        try {
            const urls = await pollRunningHubWorkflowTask(taskId, useWallet);
            delete node.rhWorkflowTaskId;
            delete node.rhWorkflowUseWallet;
            if(urls && urls.length){
                // 成功：补全节点结果并停止计时
                finalizePendingNode(node, urls, null);
                render();
                scheduleSave();
                return;
            }
            ...
            render();
            scheduleSave();
            toast((e.message || tr('smart.rhFailed')).slice(0, 160));
            return;
        }
    }
    const tasks = smartPendingTasks(node);
    if(!tasks.length){
        // 陈旧计数自愈：pending>0 却没有任何任务可轮询（页面中断/异常残留，或任务已完成但计数没被收回）。
        // 这种情况下永远无法正常收尾（也取不回输出），直接归零并结束/停止计时，避免一直走秒。
        if(Number(node.pending || 0) > 0){
            node.pending = 0;
            delete node.pendingTasks;
            node.running = false;
            node.runFinishedAt = nowMs();
...
function resumeSmartPendingTasks(){
    // 除有任务可轮询的节点外，也纳入「有 pending 但无任务」的陈旧节点，让其自愈归零、停止计时。
    nodes.filter(node => smartPendingTasks(node).length || Number(node.pending || 0) > 0 || node.rhWorkflowTaskId).forEach(node => {
...
function finishSelection(event){
    ...
    const nextSelected = new Set(selectionState.additive ? selectionState.baseSelectedIds : []);
    nodes.forEach(node => {
        const r = nodeRect(node);
        if(r.x < maxX && r.x + r.width > minX && r.y < maxY && r.y + r.height > minY) nextSelected.add(node.id);
    });
...
shell.onmousedown = e => {
    ...
    if(e.button === 0 && isRKeyDown){
        e.preventDefault();
        didPan = false;
        selectionState = {startScreen:{x:e.clientX, y:e.clientY}, startWorld:screenToWorld(e), additive:false, baseSelectedIds:[]};
    ...
    if(e.button === 0 && (e.ctrlKey || e.metaKey)){
        e.preventDefault();
        didPan = false;
        const baseSelectedIds = selectedIds.length ? selectedIds.slice() : (selectedId ? [selectedId] : []);
        selectionState = {startScreen:{x:e.clientX, y:e.clientY}, startWorld:screenToWorld(e), additive:true, baseSelectedIds};
        updateSelectionBox(e);
        return;
    }
...
shell.oncontextmenu = e => {
    ...
    if(didPan || e.target.closest('.composer,.smart-back,.asset-panel,.asset-toggle,.smart-queue-toggle,.smart-queue-panel,.smart-log-toggle,.smart-shortcut-toggle,.smart-workflow-toggle,.log-modal,.shortcut-modal,.image-edit-modal,.create-menu,.smart-minimap')) return;
...
shell.ondblclick = e => {
    if(didPan || e.target.closest('.image-node,.composer,.smart-back,.asset-panel,.asset-toggle,.smart-queue-toggle,.smart-queue-panel,.smart-log-toggle,.smart-shortcut-toggle,.smart-workflow-toggle,.log-modal,.shortcut-modal,.image-edit-modal,.create-menu')) return;
    if(document.getElementById('imageEditModal')?.classList.contains('open')) return;
    e.preventDefault();
    openCreateMenu(e);
};
...
window.onmousemove = e => {
    ...
    if(resizeState){
        ...
        if(e.shiftKey){
            const ratio = resizeState.startW / Math.max(1, resizeState.startH);
            const widthFromX = resizeState.startW + dx;
            const heightFromY = resizeState.startH + dy;
            const useWidth = Math.abs(dx) >= Math.abs(dy * ratio);
            let nextW = useWidth ? widthFromX : heightFromY * ratio;
            let nextH = nextW / ratio;
            if(nextW < minW || nextH < minH){
                const scale = Math.max(minW / resizeState.startW, minH / resizeState.startH);
                nextW = resizeState.startW * scale;
                nextH = resizeState.startH * scale;
            }
        ...
        if(!thumbDragState.detached && Math.abs(dx) + Math.abs(dy) > 6){
            ...
            if(canDetachThumb){
                ...
                if(img){
                    ...
                    } else if(source.images.length <= 1 && !source.displayTitle){
                        source.title = 'Image';
...
window.onmouseup = e => {
    ...
    if(dragState){
        ...
        // 清理拖拽态：取消 rAF、移除 dragging 类、恢复 left/top 为最终位置
        ...
        (dragState.group || [{id:dragState.id}]).forEach(item => {
            ...
            if(el){
                el.classList.remove('dragging');
                el.style.transform = '';
                // 将 left/top 恢复为最终坐标（而非清空），避免节点漂移到默认位置
                if(n){
...
function syncApiKindToggleVisibility(){
    ...
    // 同步引擎快捷按钮选中状态
    if(engineQuickToggle){
        engineQuickToggle.querySelectorAll('[data-engine]').forEach(btn => btn.classList.toggle('active', btn.dataset.engine === settings.engine));
    }
}
if(apiKindToggle){
...
if(engineQuickToggle){
    engineQuickToggle.querySelectorAll('[data-engine]').forEach(btn => {
        btn.onclick = e => {
            e.preventDefault();
            e.stopPropagation();
...
if(smartComposerQueueBtn){
    smartComposerQueueBtn.onclick = () => {
        const node = selectedNode();
        if(!node || !isSmartRunnableNode(node)){ toast('请先选择一个可运行的节点'); return; }
        queueToggleNode(node.id);
        syncSmartComposerQueueBtn(node);
        render();
        if(smartRunQueue.length && !(smartQueuePanelEl || document.getElementById('smartQueuePanel'))?.classList.contains('open')) toggleSmartQueuePanel(true);
    };
}
...
if(smartQueueToggle) smartQueueToggle.onclick = () => toggleSmartQueuePanel();
if(smartQueueClose) smartQueueClose.onclick = () => toggleSmartQueuePanel(false);
const smartQueuePanelElInit = document.getElementById('smartQueuePanel');
if(smartQueuePanelElInit){
    smartQueuePanelElInit.querySelector('.smart-queue-start')?.addEventListener('click', () => queueStart());
    smartQueuePanelElInit.querySelector('.smart-queue-clear-fin')?.addEventListener('click', () => queueClearFinished());
    smartQueuePanelElInit.querySelector('.smart-queue-clear')?.addEventListener('click', () => {
        smartRunQueue = [];
        smartQueueRuns = {};
        renderSmartQueuePanel();
        render();
...
document.addEventListener('mousedown', e => {
    if(smartNodeContextMenu && !smartNodeContextMenu.contains(e.target)) closeNodeContextMenu();
}, true);
...
// ===== 角度控制面板 =====
let angleThreeScene = null;
let angleThreeRenderer = null;
let angleThreeCamera = null;
let angleThreeMesh = null;
let angleThreeGrid = null;

/** 确保角度面板 DOM 存在（动态创建，不依赖 HTML 文件） */
function ensureAnglePanel(){
    if(document.getElementById('anglePanel')) return;
    const div = document.createElement('div');
    div.id = 'anglePanel';
    div.className = 'angle-panel';
    div.style.display = 'none';
    div.innerHTML = '<div class="angle-panel-backdrop" onclick="closeAnglePanel()"></div>'
        + '<div class="angle-panel-modal">'
        + '<div class="angle-panel-head">'
        + '<span class="angle-panel-title">角度控制</span>'
        + '<div class="angle-panel-actions">'
        + '<button type="button" class="angle-panel-btn primary" onclick="applyAngleToNode()">应用</button>'
        + '<button type="button" class="angle-panel-btn" onclick="closeAnglePanel()">关闭</button>'
        + '</div></div>'
        + '<div class="angle-panel-body">'
        + '<div class="angle-panel-3d" id="angleThreeContainer"><div class="angle-panel-3d-empty">加载图片以预览角度</div></div>'
        + '<div class="angle-panel-controls">'
        + '<div class="angle-control-group"><div class="angle-control-label"><i data-lucide="move-horizontal" class="w-3 h-3"></i><span>水平旋转</span><div class="angle-control-value"><input type="number" id="angleValH" value="0" class="angle-num-input" oninput="syncAngleInput(\'h\')"><span>°</span><button type="button" class="angle-reset-btn" onclick="resetAngleControl(\'h\')" title="重置"><i data-lucide="rotate-ccw" class="w-3 h-3"></i></button></div></div><input type="range" id="angleSliderH" min="-90" max="90" value="0" class="angle-slider"></div>'
        + '<div class="angle-control-group"><div class="angle-control-label"><i data-lucide="move-vertical" class="w-3 h-3"></i><span>俯仰</span><div class="angle-control-value"><input type="number" id="angleValV" value="0" class="angle-num-input" oninput="syncAngleInput(\'v\')"><span>°</span><button type="button" class="angle-reset-btn" onclick="resetAngleControl(\'v\')" title="重置"><i data-lucide="rotate-ccw" class="w-3 h-3"></i></button></div></div><input type="range" id="angleSliderV" min="-90" max="90" value="0" class="angle-slider"></div>'
        + '<div class="angle-control-group"><div class="angle-control-label"><i data-lucide="zoom-in" class="w-3 h-3"></i><span>距离</span><div class="angle-control-value"><input type="number" id="angleValD" value="4.0" step="0.1" class="angle-num-input" oninput="syncAngleInput(\'d\')"><button type="button" class="angle-reset-btn" onclick="resetAngleControl(\'d\')" title="重置"><i data-lucide="rotate-ccw" class="w-3 h-3"></i></button></div></div><input type="range" id="angleSliderD" min="0.1" max="8" value="4" step="0.1" class="angle-slider"></div>'
...
function syncAngleInput(axis){
    const slider = document.getElementById('angleSlider' + axis.toUpperCase());
    const val = document.getElementById('angleVal' + axis.toUpperCase());
...
function resetAngleControl(axis){
    const slider = document.getElementById('angleSlider' + axis.toUpperCase());
    const val = document.getElementById('angleVal' + axis.toUpperCase());
    if(!slider || !val) return;
    const defaults = {H:0, V:0, D:4};
    const def = defaults[axis.toUpperCase()] || 0;
    slider.value = def;
    val.value = def;
    updateAngleSceneCamera();
    updateAngleCommand();
...
function updateAngleCommand(){
    const h = parseFloat(document.getElementById('angleSliderH').value) || 0;
    const v = parseFloat(document.getElementById('angleSliderV').value) || 0;
    const d = parseFloat(document.getElementById('angleSliderD').value) || 4;
    let parts = [];
    if(h !== 0) parts.push((h > 0 ? '向右' : '向左') + '旋转' + Math.abs(h) + '度');
    if(v !== 0) parts.push((v > 0 ? '俯视' : '仰视') + Math.abs(v) + '度');
    let resultText = '';
    if(parts.length > 0) resultText = '将相机' + parts.join('，');
    if(d > 4) resultText += (resultText ? '，' : '将相机') + '使用广角镜头';
    else if(d < 4) resultText += (resultText ? '，' : '将相机') + '使用特写镜头';
    document.getElementById('angleCommandPreview').textContent = resultText || '将相机保持原位';
    // 实时更新 3D 预览
    if(angleThreeScene) updateAngleSceneCamera();
}

function updateAngleSceneCamera(){
    const h = parseFloat(document.getElementById('angleSliderH').value) || 0;
    const v = parseFloat(document.getElementById('angleSliderV').value) || 0;
    const d = parseFloat(document.getElementById('angleSliderD').value) || 4;
    if(!angleThreeCamera) return;
    const phi = (90 - v) * Math.PI / 180;
    const theta = h * Math.PI / 180;
    angleThreeCamera.position.x = d * Math.sin(phi) * Math.sin(theta);
    angleThreeCamera.position.y = d * Math.cos(phi);
    angleThreeCamera.position.z = d * Math.sin(phi) * Math.cos(theta);
    angleThreeCamera.lookAt(0, 0, 0);
}

async function initAngleScene(imgUrl){
    let THREE = window.THREE;
    if(!THREE){
        try {
            THREE = await import('/static/vendor/js/three-0.160.0.module.js');
            window.THREE = THREE;
        } catch(e){
            console.error('角度面板: THREE.js 加载失败', e);
            return setTimeout(() => initAngleScene(imgUrl), 200);
        }
    }
    const container = document.getElementById('angleThreeContainer');
    if(!container) return;
    // 清理旧场景
    disposeAngleScene();
    // 空状态
    const empty = container.querySelector('.angle-panel-3d-empty');
    if(empty) empty.style.display = imgUrl ? 'none' : 'flex';
    // 创建场景
    const scene = new THREE.Scene();
    scene.background = new THREE.Color(0x222222);
    angleThreeScene = scene;
    // 相机
    const camera = new THREE.PerspectiveCamera(75, container.clientWidth / container.clientHeight, 0.1, 1000);
    angleThreeCamera = camera;
    // 渲染器
    const renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true });
    renderer.setSize(container.clientWidth, container.clientHeight);
    renderer.setPixelRatio(window.devicePixelRatio);
    container.appendChild(renderer.domElement);
    angleThreeRenderer = renderer;
    // 网格
    ...
    if(imgUrl){
        new THREE.TextureLoader().load(imgUrl, (texture) => {
            ...
            if(imageAspect > 1){
                ...
                mesh.geometry = new THREE.PlaneGeometry(3, 3 / imageAspect);
            } else {
                mesh.geometry.dispose();
                mesh.geometry = new THREE.PlaneGeometry(3 * imageAspect, 3);
            }
    ...
    angleThreeRenderer._resizeObserver = ro;
}

function disposeAngleScene(){
    if(angleThreeRenderer && angleThreeRenderer._resizeObserver){
        angleThreeRenderer._resizeObserver.disconnect();
    }
    if(angleThreeRenderer && angleThreeRenderer.domElement && angleThreeRenderer.domElement.parentNode){
        angleThreeRenderer.domElement.parentNode.removeChild(angleThreeRenderer.domElement);
    }
    if(angleThreeRenderer) angleThreeRenderer.dispose();
    angleThreeScene = null;
    angleThreeRenderer = null;
    angleThreeCamera = null;
    angleThreeMesh = null;
    angleThreeGrid = null;
}
...
function applyAngleToNode(){
    ...
    const d = parseFloat(document.getElementById('angleSliderD').value) || 4;
    // 保存到 settings
    settings.cameraAngle = {h, v, d};
    // 生成相机命令文本
    let parts = [];
    if(h !== 0) parts.push((h > 0 ? '向右' : '向左') + '旋转' + Math.abs(h) + '度');
    if(v !== 0) parts.push((v > 0 ? '俯视' : '仰视') + Math.abs(v) + '度');
    let cmd = '';
    if(parts.length > 0) cmd = '将相机' + parts.join('，');
    if(d > 4) cmd += (cmd ? '，' : '将相机') + '使用广角镜头';
    else if(d < 4) cmd += (cmd ? '，' : '将相机') + '使用特写镜头';
    // 注入到节点 prompt
    if(cmd){
        const currentPrompt = node.promptDraftText || '';
        const regex = /将相机.*?(?=(\n|$))/g;
        if(regex.test(currentPrompt)){
            node.promptDraftText = currentPrompt.replace(regex, cmd);
        } else {
            if(currentPrompt.trim()){
                node.promptDraftText = currentPrompt.trim() + '\n' + cmd;
            } else {
    ...
    // 更新渲染
    if(node.runPrompt) delete node.runPrompt;
    render();
    saveCanvas();
    // 刷新 settings 面板
    renderApiParams();
